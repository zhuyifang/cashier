...
class ShareApplyForm extends Model
{
    ...
    public function getShareStatus()
    {
        ...
        return [
            ...
            'data' => [
                ...
                'template_message' => $templateMessage,
                'becomeCondition' => $this->becomeCondition,
            ]
        ];
    }
    ...
    public function checkApply()
    {
        ...
        $isCondition = ShareSetting::get($this->mall->id, ShareSetting::IS_CONDITION, 1);
        if ($isCondition == 0) {
            return true;
        }
        $newBecomeCondition = ShareSetting::get($this->mall->id, ShareSetting::NEW_BECOME_CONDITION);
        $consumeCondition = ShareSetting::get($this->mall->id, ShareSetting::CONSUME_CONDITION);
        foreach ($newBecomeCondition as $becomeCondition) {
            try {
                switch ($becomeCondition) {
                    case 'one_consume':
                        $flag = $this->checkOrder($consumeCondition);
                        break;
                    case 'buy_goods':
                        switch ($consumeCondition) {
                            case 1:
                                $flag = $this->checkGoods();
                                break;
                            case 2:
                                $flag = $this->checkGoodsBySale();
                                break;
                            default:
                                $flag = false;
                        }
                        break;
                    case 'total_consume':
                        $flag = $this->checkTotalOrder($consumeCondition);
                        break;
                    default:
                        $flag = false;
                }
                if ($flag) {
                    $this->becomeCondition = $becomeCondition;
                    return true;
                }
            } catch (\Exception $exception) {
                \Yii::error($exception);
    ...
    protected function checkTotalOrder($consumeCondition)
    {
        ...
        switch ($consumeCondition) {
            case 1:
                $this->totalOrderPrice = CommonShare::getCommon($this->mall)->getTotalConsume();
                break;
            case 2:
                ...
                $this->totalOrderPrice = round($orderPrice, 2);
                break;
            default:
                return false;
        }
        if ($totalConsume > $this->totalOrderPrice) {
            throw new \Exception('未满足累计消费' . $totalConsume . '元的条件，不能申请分销商');
        }
        return true;
    }

    public function getBecomeCondition()
    {
        return $this->becomeCondition;
    }
}
