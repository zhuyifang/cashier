...
//            ->asArray()
//            ->one();
        $this->list['pay_price'] = 0;
        $this->list['pay_num'] = 0;
        $this->list['coupon_discount_price'] = 0;
        $this->list['member_discount_price'] = 0;
        foreach ($order_info as $item) {
            $this->list['pay_price'] += $item['total_pay_price'];
            $this->list['pay_num'] += $item['all_num'];
            $this->list['coupon_discount_price'] += $item['coupon_discount_price'];
            $this->list['member_discount_price'] += $item['all_member_discount_price'];
        }
        $this->list['pay_price'] = price_format($this->list['pay_price']);
        $this->list['pay_num'] = price_format($this->list['pay_num']);
        $this->list['coupon_discount_price'] = price_format($this->list['coupon_discount_price']);
        $this->list['member_discount_price'] = price_format($this->list['member_discount_price']);
    ...
    private function order_where()
    {
        $model = Order::find()->alias('o')
            ->select([
                'o.coupon_discount_price', 'o.total_pay_price',
                'sum(od.num) as all_num',
                'sum(od.member_discount_price) as all_member_discount_price'
            ])
            ->leftJoin(['od' => OrderDetail::tableName()], 'od.order_id = o.id')
            ->where([
                'o.is_delete' => 0, 'o.is_pay' => 1, 'o.mall_id' => \Yii::$app->mall->id,
        ...
        try {
            ...
            $model->leftJoin(['vcd' => VipCardDiscount::tableName()], 'vcd.order_id = o.id')
                ->select([
                    'o.coupon_discount_price', 'o.total_pay_price',
                    'sum(od.num) as all_num',
                    '(sum(od.member_discount_price) + IFNULL(sum(vcd.discount),0)) as all_member_discount_price'
                ]);
        } catch (\Exception $exception) {
...
//                }
                ...
                try {
                    if ($item['sign'] !== '') {
                        ...
                        $item['is_negotiable'] = $item['mallGoods']['is_negotiable'];
                    }
                } catch (\Exception $exception) {
                    \Yii::error($exception);
                }
