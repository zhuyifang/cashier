 ...
 * @property int $is_share 是否允许分销：0=否，1=是
...
class OrderGoodsAttr extends Model
{
    ...
    {
        return [
            [['goods_id', 'stock', 'weight', 'id', 'goods_warehouse_id', 'use_integral', 'number',
                'is_share'], 'integer'],
            [['price', 'original_price', 'member_price', 'integral_price', 'cost_price'], 'number', 'min' => 0],
            [['sign_id', 'no', 'bar_code', 'pic_url', 'name', 'cover_pic'], 'string', 'max' => 255],
            [['extra', 'discount'], 'safe']
        ];
    }
    ...
    public function setGoods($goods)
    {
        ...
        $this->detail = '';
        $this->pic_list = json_decode($goods->goodsWarehouse->pic_url, true);
    ...
    public function setGoodsAttr($goodsAttr)
    {
        ...
        // 兼容商品单规格成本价错误
        if (!$this->goods->use_attr) {
            $goodsAttr->cost_price = $this->goods->goodsWarehouse->cost_price;
        }
    ...
    public function setShare()
    {
        ...
        $this->is_share = $this->goods->is_share;
