...
use app\forms\common\send_type\SendType;
...
class OrderEditForm extends Model
{
    ...
    public function getDetail()
    {
        try {
            ...
            $order['rest_auto_cancel_time'] = strtotime($order['auto_cancel_time']) - time();

            $clerkUser = (new Order())->checkClerkPermission($order['store_id'], \Yii::$app->user->id, $order['mch_id']);
            $order['is_clerk_permission'] = $clerkUser ? true : false;
            $order['send_type_text'] = SendType::getSendType($order['send_type']);
            $order = array_merge($order, CommonCustomization::getCommon()->getBtnName($order['detail']));

            $newOrder = Order::findOne($order['id']);
            $order['sign_name'] = $newOrder->getSignName();
            return [
