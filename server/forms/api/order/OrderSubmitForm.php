...
use app\forms\common\balance\BalanceCreate;
...
class OrderSubmitForm extends Model
{
    ...
    public function preview()
    {
        ...
        try {
            ...
            $data['address'] = $this->addressEncode($data['address']);
    ...
    public function submit()
    {
        ...
        try {
            ...
            $this->checkSupportPayTypes($data);
        } catch (OrderException $orderException) {
            return [
                'code' => 1,
                'msg' => $orderException->getMessage(),
    ...
    protected function checkAddress($data)
    {
        if (!$data['hasCity'] || count($data['mch_list']) >= 2) {
            if (!$this->getAddress() && !$data['allZiti']) {
                throw new OrderException('请先选择收货地址。');
            }
        }
        if ($data['hasCity'] && (!isset($data['mch_list'][0]['address']) || !$data['mch_list'][0]['address'])) {
            throw new OrderException('请先选择配送地址。');
        }
        if ($data['allZiti'] && $data['hasRecipient']) {
            if (!$data['address']['name']) {
                throw new OrderException('请填写联系人');
            }
            if (!$data['address']['mobile']) {
                throw new OrderException('请填写手机号');
            }
            if (isset($data['address']['mobile'])) {
                // 验证手机号
                $mobile = $data['address']['mobile'];
                if (\Yii::$app->mall->getMallSettingOne('is_verify_mobile') && $mobile) {
                    if (mb_strlen($mobile) != 11 || substr($mobile, 0, 1) != 1) {
                        throw new OrderException('手机号码错误');
                    }
                }
            }
        }
        ...
        foreach ($data['mch_list'] as $mchItem) {
            if (isset($mchItem['mch']['id']) && $mchItem['mch']['id'] > 0) {
                $mchIdList[] = $mchItem['mch']['id'];
            }
            if (isset($mchItem['address']['mobile'])) {
                $mobile = $mchItem['address']['mobile'];
                if (\Yii::$app->mall->getMallSettingOne('is_verify_mobile') && $mobile) {
                    if (mb_strlen($mobile) != 11 || substr($mobile, 0, 1) != 1) {
                        throw new OrderException('手机号码错误');
                    }
            ...
            if (isset($mchItem['city']) && isset($mchItem['city']['error'])) {
                throw new OrderException($mchItem['city']['error']);
            }
        }
    }

    protected function checkSupportPayTypes($data)
    {
        $balanceCreate = BalanceCreate::create();
        if (isset($data['hasEcard']) && $data['hasEcard']) {
            $this->supportPayTypes = $balanceCreate->unsetPayType($this->supportPayTypes, 'huodao');
        }

        if (!$balanceCreate->isBalance()) {
            // 余额功能未开启，去掉余额支付
            $this->supportPayTypes = $balanceCreate->unsetPayType($this->supportPayTypes, 'balance');
        }
    }
    ...
    public function getAllData()
    {
        ...
        $shopStatus = (new IndexForm())->getShopStatus($mchIdList);
        foreach ($shopStatus as $item) {
            if ($item['is_open'] == 2) {
                return [
                    'code' => 1,
                    'msg' => '商家已打烊'
    ...
    protected function getMchListData($formMchList)
    {
        ...
        foreach ($formMchList as $i => $formMchItem) {
            $formData = $this->getFormMchItem($formMchItem);
            $mchItem = [
    ...
    protected function getFormMchItem($formMchItem)
    {
        return $formMchItem;
    }
    ...
    {
        $goods = $this->getGoods($item);
        // 其他商品特有判断
        $this->checkGoods($goods, $item);
        $this->checkGoodsWhite($goods, $item);

        try {
            $goodsAttr = $this->getGoodsItemAttr($item, $goods);
        ...
        $itemData = [
            ...
            'member_price' => $goodsAttr->member_price,
    ...
    protected function getGoodsItemAttr($item, $goods)
    {
    ...
    protected function setMemberDiscountData($mchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            if ($memberUnitPrice && is_numeric($memberUnitPrice) && $memberUnitPrice >= 0) {
                ...
                $goodsItem['member_price'] = price_format($memberUnitPrice);
                if ($memberSubPrice != 0) {
                    // 减去会员优惠金额
    ...
    protected function setIntegralDiscountData($mchItem, $formMchItem)
    {
        ...
        $mchItem['integral']['can_use'] = $mchItem['integral']['use_num'] > 0;
        $mchItem['integral']['use'] = $formMchItem['use_integral'] == 1;
     ...
     * 到店自提运费
     */
    protected function setOfflineExpressData($mchItem)
    {
        return $mchItem;
    }
    ...
    {
        $address = $this->getCityAddress($mchItem);
    ...
    protected function setExpressData($mchItem)
    {
        ...
        switch ($mchItem['delivery']['send_type']) {
            case 'offline':
                $mchItem = $this->setOfflineExpressData($mchItem);
                break;
            case 'city':
                $mchItem = $this->setCityExpressData($mchItem);
                break;
            case 'express':
                $mchItem = $this->setExpressExpressData($mchItem);
                break;
            default:
                throw new OrderException('错误的发货方式');
        }
    ...
    public function getUsableCouponList($mchData)
    {
        ...
        if (\Yii::$app->request->post('is_cant_use_list') == 1) {
            ...
            foreach ($newList as $value) {
                $mchData['user_coupon_id'] = $value['id'];
                $this->form_data = [
                    'list' => [$mchData],
                    'address_id' => 0,
                    'send_type' => ''
                ...
                $priceList[] = $allData['total_price'];
                $tempList[] = $value;
                $tempList = $this->sortArray($tempList, $priceList);
            }
            return $tempList;
        }
    }

    protected function sortArray($list, &$priceList)
    {
        if (count($list) == 1) {
            return $list;
        }
        for ($i = count($list) - 1; $i >= 1; $i--) {
            if ($priceList[$i] < $priceList[$i - 1]) {
                ...
                $list[$i] = $list[$i - 1];
                $list[$i - 1] = $temp;
                $tempPrice = $priceList[$i];
                $priceList[$i] = $priceList[$i - 1];
                $priceList[$i - 1] = $tempPrice;
            } else {
                return $list;
            }
        }
    ...
    public function subGoodsNum($goodsAttr, $subNum, $goodsItem)
    {
        ...
        (new GoodsAttr())->updateStock($subNum, 'sub', $goodsAttr->id,$goodsItem,$this->sign);
    }
    ...
    public function extraGoodsDetail($order, $goodsItem)
    {
        ...
        $goodsInfo = [
            ...
            'goods_attr' => ArrayHelper::toArray($goodsItem['goods_attr']),
        ];
    ...
    protected function getAddressEnable($address, &$mchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$item) {
            ...
            //如果是插件商品且插件关闭了区域允许设置 套餐商品虽然标示是商城商品但是也需要被套餐插件中的开关控制
            if (($goods->sign != '' || $this->sign == 'composition') && !$this->isGoodsEnableAddressLimit($item)) {
                continue;
            }
    ...
    protected function getGoods($item)
    {
        /** @var Goods $goods */
    ...
    private function setCoupon($mchItem, $totalGoodsPrice, $subPrice, $couponGoodsIdList = null)
    {
        ...
        foreach ($mchItem['goods_list'] as $goods) {
            if (
                $couponGoodsIdList && !empty($couponGoodsIdList)
        ...
        $tempList = [];
        foreach ($newGoodsList as $index => &$goods) {
            /* @var float $goodsPrice 商品可优惠的金额 */
            $goodsPrice = price_format($goods['total_price'] * $subPrice / $totalGoodsPrice);
            if ($resetPrice < $goodsPrice || ($index == count($newGoodsList) - 1 && $resetPrice > 0)) {
                $goodsPrice = price_format($resetPrice);
            }
            $resetPrice -= $goodsPrice;
            $goods['total_price'] -= min($goodsPrice, $goods['total_price']);
            $tempList[($goods['goods_attr'])->id] = $goods;
        }
        unset($goods);
        // 根据旧商品列表顺序或得新的商品列表start--->
        $goodsList = [];
        foreach ($originGoodsList as &$originGoods) {
            if (isset($tempList[($originGoods['goods_attr'])->id])) {
                $goodsList[] = $tempList[($originGoods['goods_attr'])->id];
            } else {
                $goodsList[] = $originGoods;
            }
        }
     ...
     * 预览接口对地址进行加密处理
     */
    public function addressEncode($address)
    {
        if (!$address) {
            return null;
        }
        if (version_compare(\Yii::$app->appVersion, '4.4.9', '<')) {
            return $address;
        }
        return base64_encode(Json::encode($address, JSON_UNESCAPED_UNICODE));
    }
    ...
    public function getNeedPostageGoodsList($mchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as $goodsItem) {
            ...
            $condition = $this->getCondition($freeDelivery, $mchItem['address']);
            if ($condition == -1) {
                $needPostageGoodsList[] = $goodsItem;
                continue;
            }
