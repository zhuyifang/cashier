...
abstract class BaseFinanceCashForm extends Model
{
    ...
    public function rules()
    {
        return [
            [['price', 'type'], 'required'],
            [['price'], 'number', 'min' => 0],
            [['type', 'name', 'mobile', 'bank_name', 'qrcode'], 'trim'],
            [['type', 'name', 'mobile', 'bank_name', 'qrcode'], 'string'],
            [['use_qrcode'], 'default', 'value' => 0],
            [['type'], function ($attr, $params) {
                if (!in_array($this->type, (array)$this->setting[Finance::PAY_TYPE])) {
                    $this->addError($attr, '错误的提现方式');
                }
            }],
            [['name', 'mobile', 'bank_name', 'qrocde'], function ($attr, $params) {
                if (
                    in_array($this->type, ['wechat', 'alipay'])
                    && in_array($attr, ['name', 'mobile'])
                    && $this->use_qrcode == 0
                    && !$this->$attr
                ) {
                ...
                if ($this->type == 'bank' && in_array($attr, ['name', 'mobile', 'bank_name']) && !$this->$attr) {
                    $this->addError($attr, $this->attributeLabels()[$attr] . '不能为空');
                }
    ...
    public function job()
    {
        try {
            ...
            $cash->use_qrcode = $this->use_qrcode;
