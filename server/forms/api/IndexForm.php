...
class IndexForm extends Model
{
    ...
    public function shopStatus()
    {
        try {
            if ($this->mch_id_list) {
                $mchIdList = is_array($this->mch_id_list) ? $this->mch_id_list : json_decode($this->mch_id_list, true);
            } else {
                $mchIdList = [];
            }
            
            $data = $this->getShopStatus($mchIdList);
            return [
                'code' => ApiCode::CODE_SUCCESS,
                'msg' => '请求成功',
                'data' => $data
    ...
    public function getShopStatus(array $mchIdList)
    {
        $mall = new Mall();
