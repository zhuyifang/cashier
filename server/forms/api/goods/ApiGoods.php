...
class ApiGoods extends Model
{
    ...
    public function getDetail()
    {
        ...
        if ($this->goods->goodsWarehouse->type == 'form-goods') {
            $minPrice = CommonCustomization::getCommon()->getDateMinPrice($this->goods);
        } else {
            $minPrice = $this->getMinPrice();
        }

        $goodsStock = $this->goods->goods_stock;
        ...
        $data = [
            'id' => $this->goods->id,
            'min_number' => $this->goods->min_number,
     ...
     * 获取会员价
     */
    public function getGoodsMember($user = null)
    {
    ...
    {
        if ($isNegotiable == 1) {
            $priceContent = \Yii::$app->mall->getMallSettingOne('negotiable_text');
        } elseif ($minPrice > 0) {
