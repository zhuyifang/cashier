...
class Price implements BaseConst
{
    ...
    {
        $negotiableText = \Yii::$app->mall->getMallSettingOne('negotiable_text');
        if ($this->isNegotiable($goods)) {
            $this->handleCenter($negotiableText, 48);
            return [
                $this->setText($negotiableText, $this->left, $this->top, 48, $this->color)
            ];
        }

        $prices = $this->getPrices($goods);
     ...
     * 判断是否价格面议
     */
    protected function isNegotiable(Goods $goods)
    {
        return $goods->mallGoods->is_negotiable ?? 0;
    }
     ...
     * 获取价格最大值及最小值
     */
    protected function getPrices(Goods $goods)
    {
