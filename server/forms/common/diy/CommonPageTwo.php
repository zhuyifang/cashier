...
use app\forms\common\diy\DiyMchGoodsForm;
use app\forms\common\diy\DiyMchHomeForm;
use app\forms\common\diy\DiyMenuForm;
...
class CommonPageTwo extends Model
{
    ...
    private $format = [
        ...
        'step' => 'sStep',
        'form-module' => 'sFormModule',
        'mch-home' => 'sMchHome',
        'time' => 'sTime',
        'button' => 'sButton',
        'weekly-buy' => 'sWeeklyBuy',
        'menu' => 'sMenu'
    ];
    //商品
    private $diyGoods = [];
    ...
    //周期购
    private $weeklyBuyGoods;

    public static function getCommon($mall, $longitude = null, $latitude = null, $mch = null)
    {
        $common = new self();
        $common->mall = $mall;
    ...
    {
        $query = DiyPage::find()->select('id,title,show_navs,is_home_page,access_limit,link,is_link')
            ->where([
                'mall_id' => $this->mall->id,
                'is_disable' => 0,
                'is_delete' => 0,
...
//            $query->findInSet(\Yii::$app->appPlatform, 'platform');
            ...
            if (isset($nav['template']['data'])) {
                ...
                if ($access_limit && is_array($access_limit) && intval($access_limit['is_all']) !== 1) {
                    ...
                    if (!\Yii::$app->user->identity || !in_array(\Yii::$app->user->identity->identity->member_level, $levels)) {
                        ...
                        $link = \yii\helpers\BaseJson::decode($this->model['link']);
                        if ($this->model['is_link'] == 1 && $link && !empty($link)) {
                            $model['link'] = $link;
                        }
                        continue;
                    }
    ...
    //导航
    public function sNav(bool $is_show, array $arr)
    {
        if ($is_show) {
            $app_admin = true;
            if (
    ...
    //商品
    public function sGoods(bool $is_show, array $arr)
    {
        if ($is_show) {
            ...
            $mchId = $this->mch ? $this->mch->id : 0;
            $this->diyGoods = $diyGoodsForm->getGoodsById($goodsIds, $mchId);
            $this->diyCatsGoods = $diyGoodsForm->getGoodsByCat($goodsCats, $mchId);
        }
    }

    // 图片广告
    public function sRubik(bool $is_show, array $arr)
    {
        if ($is_show) {
            foreach ($arr['data']['list'] as $rIndex => $rItem) {
                try {
                    ...
                    if ($arr['data']['style'] == 0 && !isset($arr['data']['a_height'])) {
                        $arr['data']['list'][$rIndex]['height'] = '750px';
                        $arr['data']['height'] = '750px';
                    }
                } catch (\Exception $exception) {
                }
    ...
    //视频
    public function sVideo(bool $is_show, array $arr)
    {        
        if ($is_show) {
    ...
    // 轮播图
    public function sBanner(bool $is_show, array $arr)
    {
        if ($is_show) {
    ...
    protected function sFormModule(bool $is_show, array $arr)
    {
        $other = ['banner', 'rubik', 'video', 'copyright', 'time', 'button'];
        if ($is_show) {
            $diyFormModuleForm = new CommonFormModule();
            $data = $diyFormModuleForm->getNewForm($arr['data'], $this->formModules);

            $this->buttonData = null;
            foreach ($data['list'] as $item) {
                if ($item['id'] == 'button') {
                    $this->buttonData = $item;
                }
            }

            $this->formId = $data['form_id'];
            foreach ($data['list'] as $key => $info) {
                if (in_array($info['id'], $other)) {
                    $keyword = $this->format[$info['id']];
                    $keyword && $data['list'][$key] = $this->$keyword(true, $info);
                }
            }
            $arr['data'] = $data;
            return $arr;
            ...
            $formModuleIds = [];
            foreach ($arr as $item) {
                array_push($formModuleIds, $item['data']['form_id']);
            }
            $this->formModules = $diyFormModuleForm->getFormById($formModuleIds);
            foreach ($this->formModules as $form) {
                foreach ($form['list'] as $info) {
                    if (in_array($info['id'], $other)) {
                        $keyword = $this->format[$info['id']];
                        $keyword && $this->$keyword(false, [$info]);
                    }
                }
            }
        }
    ...
    //商品
    public function sMchGoods(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyGoodsForm = new DiyMchGoodsForm();
            $arr['data'] = $diyGoodsForm->getNewGoods($arr['data'], $this->diyGoods);
            return $arr;
        } else {
            $diyGoodsForm = new DiyMchGoodsForm();
            $goodsIds = [];
            foreach ($arr as $goods) {
                $goodsIds = array_merge($diyGoodsForm->getGoodsIds($goods), $goodsIds);
            }
    ...
    //商品
    public function sWeeklyBuy(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyGoodsForm = new DiyWeeklyBuyForm();
            $arr['data'] = $diyGoodsForm->getNewGoods($arr['data'], $this->weeklyBuyGoods);
            return $arr;
        } else {
            $diyGoodsForm = new DiyWeeklyBuyForm();
            $goodsIds = [];
            ...
            $this->weeklyBuyGoods = $diyGoodsForm->getGoodsById($goodsIds);
        }
    }

    public function sMchHome(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyGoodsForm = new DiyMchHomeForm();
            $arr['data'] = $diyGoodsForm->getNewData($arr['data'], $this->mch);
            return $arr;
        }
    }

    //实时动态
    public function sTime(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyGoodsForm = new DiyTimeForm();
            $arr['data'] = $diyGoodsForm->getNewData($arr['data'], $this->formId, $this->buttonData);
            return $arr;
        }
    }

    //按钮
    public function sButton(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyGoodsForm = new DiyButtonForm();
            $arr['data'] = $diyGoodsForm->getNewData($arr['data'], $this->formId);
            return $arr;
        }
    }

    //下拉菜单
    public function sMenu(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyGoodsForm = new DiyMenuForm();
            $arr['data'] = $diyGoodsForm->getNewData($arr['data']);
            return $arr;
        }
    }
}
