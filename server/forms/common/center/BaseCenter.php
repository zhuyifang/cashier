...
class BaseCenter extends Component
{
    ...
    // todo 开发完成此处请切换
    // private $xBdUrlPro = 'aHR0cHM6Ly9iZGF1dGguempoZWppYW5nLmNvbQ=='; // 正式
    private $xBdUrlDev = 'aHR0cDovL2xvY2FsaG9zdC9iZGF1dGgvd2Vi'; // 开发
     ...
     * @throws Exception
     */
    public function xHttpGet($url, $params = [])
    {
        $url = $this->getUrl($url);
        $url = $this->appendParams($url, $params);
        $body = $this->xBdRequest('get', $url);
     ...
     * @throws Exception
     */
    public function xHttpPost($url, $params = [], $data = [])
    {
        $url = $this->getUrl($url);
        $url = $this->appendParams($url, $params);
        $body = $this->xBdRequest('post', $url, $data);
        $res = json_decode($body, true);
        if (!$res) {
            throw new \Exception('Cloud response body `' . $body . '` could not be decode.');
        }
    ...
    private function getUrl($url)
    {
        ...
        $baseUrl = base64_decode($this->xBdUrlPro);
     ...
     * @throws \Exception
     */
    private function xBdRequest($method, $url, $data = null)
    {
        try {
            $version = app_version();
        } catch (\Exception $e) {
