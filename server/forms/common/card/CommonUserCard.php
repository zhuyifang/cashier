...
class CommonUserCard extends Model
{
    ...
    public function detail()
    {
        $userCard = UserCard::find()
            ...
            ->with(['clerk', 'store', 'order', 'detail', 'card.storeRelation', 'user.userInfo'])
            ->one();
    ...
    public function clerk()
    {
        ...
        try {
            ...
            if ($userCard->card->use_type == 1) {
                $storeIds = [];
                foreach ($userCard->card->storeRelation as $item) {
                    $storeIds[] = $item->store_id;
                }

                if (!in_array($clerkUser->storeRelation->store_id, $storeIds)) {
                    throw new Exception('没有核销权限，禁止核销');
                }
            }
