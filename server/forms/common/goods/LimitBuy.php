...
class LimitBuy extends Model
{
    ...
    public function getLimitBuy()
    {
        ...
        $list = [
            'all' => $this->goods->confine_count, // 永久限购
            $this->goods->limit_buy_type => $this->goods->limit_buy_value, // 每天、每周、每月限购
            'order' => $this->goods->limit_buy_order, // 每个订单限购
        ];
        if (max($list) <= 0 || $this->goods->limit_buy_status == 0) {
            $res['status'] = 0;
            return $res;
        }
        // 查询用户总共购买商品数
