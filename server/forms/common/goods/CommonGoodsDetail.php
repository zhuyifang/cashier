...
class CommonGoodsDetail extends Model
{
    ...
    public function getGoodsMarketing()
    {
        ...
        $limit = '';
        if ($this->isLimit && !in_array($this->goods->goodsWarehouse->type, ['ecard', GoodsWarehouse::TYPE_VIRTUAL], true)) {
...
//$pickup = substr($pickup, -1) == ',' ? substr($pickup, 0, -1) : $pickup;
        ...
        //包邮
        $shipping = '';
        if ($this->isShipping && $this->goods->is_shipping == 1) {
            try {
...
/* @var GoodsCardRelation $item */
    ...
    public function getAll($params = [], $ignore = [])
    {
        ...
        $userMember = null;
        if ($this->user) {
            $userMember = MallMembers::find()->andWhere([
        ...
        $result['form_goods'] = CommonCustomization::getCommon()->getFormGoods($this->goods->id);
        $result['attr'] = CommonCustomization::getCommon()->handleDate($result['attr'], $userMember, $this->goods, $this->user);
        if ($this->goods && $this->goods->goodsWarehouse->type == 'form-goods') {
            $result['goods_num'] = $result['goods_stock'];
        }
        return $result;
    }
    ...
    public function isVideoNumber()
    {
        ...
        if ($this->user) {
            ...
            if ($data['is_video_number_member']) {
                ...
                $levelList = [];
                foreach ($data['video_number_member_list'] as $item) {
                    $levelList[] = $item['level'];
                }

                $share = Share::find()->andWhere(['user_id' => $this->user->id])->one();

                if (!$share) {
                    $isVideoNumber = false;
                }

                if ($share && !in_array($share->level, $levelList) && !in_array(-1, $levelList)) {
                    $isVideoNumber = false;
                }
            }
        }
...
/* @var GoodsAttr[] $goodsAttr */
    ...
    public function getShare()
    {
        ...
        if ($this->goods->is_share == 0 && $this->goods->mch_id == 0) {
            return 0;
        }
        if (!$this->isShare) {
            return 0;
