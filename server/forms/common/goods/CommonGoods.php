...
class CommonGoods extends Model
{
    ...
    public function getGoods($id, $asArray = false)
    {
        ...
        $query = Goods::find()->with(['attr.share', 'attr.memberPrice', 'attr.shareLevel', 'goodsWarehouse.cats', 'attr'])
    ...
    public function getGoodsDetail($id, $hasGoodsWarehouse = true)
    {
        ...
        foreach ($detail['attr'] as $v) {
            foreach ($v['attr_list'] as $w) {
                $aKey = $w['attr_group_id'] . '_' . $w['attr_id'];
                if (!isset($temp[$aKey])) {
                    $temp[$aKey] = $v['pic_url'];
                }
            }
        ...
        foreach ($detail['attr_groups'] as $k => $v) {
            foreach ($v['attr_list'] as $l => $w) {
                $aKey = $v['attr_group_id'] . '_' . $w['attr_id'];
                $detail['attr_groups'][$k]['attr_list'][$l]['pic_url'] = $temp[$aKey] ?? "";
            }
        }
        ...
        $formGoods = CommonCustomization::getCommon()->getFormGoods($goods->id);
        $detail['date'] = isset($formGoods['date_share_data']) ? $formGoods['date_share_data'] : [];
        $detail = array_merge($detail, $formGoods);
    ...
    public function transformAttr($goods)
    {
        ...
        $calendarData = CommonCustomization::getCommon()->getCalendarData($goods->id);
        ...
        foreach ($goods->attr as $key => $attrItem) {
            ...
            $detail['attr'][$key]['date'] = isset($calendarData[$attrItem['id']]) ? $calendarData[$attrItem['id']] : [];
    ...
    public function setGoodsPayment($order, $type = 'sub')
    {
        ...
        $list = OrderDetail::find()->alias('od')->where(['od.goods_id' => $goodsIdList, 'od.is_delete' => 0])
            ...
            ->groupBy('od.goods_id')
            ->all();
        $goodsList = $this->getGoodsList($order);
