...
class CommonGoodsList extends Model
{
    ...
    public $is_show_form_goods = true;
    public $cat_id;
    ...
    protected function getMethod($key)
    {
        $array = [
            ...
            'is_show' => 'setShowAuth',
            'is_show_form_goods' => 'isShowFormGoods'
        ];
        return isset($array[$key]) ? $array[$key] : null;
    }

    protected function pluginWhere(){
        if ($this->sign) {
    ...
    {
        $query = GoodsWarehouse::find()->andWhere(['is_delete' => 0])
            ->keyword($this->keyword !== '', ['or', ['like', 'name', $this->keyword], ['like', 'keyword', $this->keyword]]);
        $goodsIds = $query->select('id');
        $this->query->andWhere([
            'or',
    ...
    public function isShowFormGoods()
    {
        if (!$this->is_show_form_goods) {
            $goodsWarehouseId = GoodsWarehouse::find()->where([
                'mall_id' => \Yii::$app->mall->id, 'is_delete' => 0,
            ])
                ->andWhere(['!=', 'type', 'form-goods'])
                ->select('id');
            $this->query->andWhere(['goods_warehouse_id' => $goodsWarehouseId]);
        }
        return $this;
