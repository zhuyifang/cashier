...
class CommonShare extends Model
{
            ...
            foreach ($newBecomeCondition as $becomeCondition) {
                try {
                    switch ($becomeCondition) {
                        case 'one_consume':
                            $flag = $this->becomeShareByConsume($order);
                            break;
                        case 'buy_goods':
                            $flag = $this->becomeShareByBuyGoods($order);
                            break;
                        case 'total_consume':
                            $flag = $this->becomeShareByTotalConsume($order, $scene);
                            break;
                        default:
                            $flag = false;
                    }
    ...
    {
        $shareCondition = ShareSetting::get($this->mall->id, ShareSetting::SHARE_CONDITION, 1);
        $isCondition = ShareSetting::get($this->mall->id, ShareSetting::IS_CONDITION);
        if ($shareCondition != 5 || $isCondition != 0) {
            return false;
        }
        $attributes = [
            'status' => 1,
    ...
    public function getIsShare()
    {
        ...
        $shareLevel = ShareSetting::get(\Yii::$app->mall->id, ShareSetting::LEVEL, 0);
        if (!$shareLevel) {
            $isShare = -1;
        }
        return $isShare;
