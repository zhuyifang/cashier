...
class AddShareOrder extends Model
{
    ...
    public function save($order, $isSendTemplate = true)
    {
        ...
        foreach ($orderDetails as $orderDetail) {
            ...
            if (
                (isset($goodsInfo['goods_attr']['is_share']) && $goodsInfo['goods_attr']['is_share'] == 0)
                || (!isset($goodsInfo['goods_attr']['is_share']) && $goodsShareSetting->is_share == 0)
            ) {
                continue;
            }
            $first = [
            ...
            $shareType = 0;
            if (
                (isset($goodsInfo['goods_attr']['individual_share']) && $goodsInfo['goods_attr']['individual_share'] == 1)
                || $goodsShareSetting->individual_share == 1
            ) {
                // 单独设置
