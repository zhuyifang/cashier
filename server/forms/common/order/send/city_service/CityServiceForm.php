...
use app\forms\common\order\send\city_service\uu\Uu;
...
class CityServiceForm
{
    ...
    public function getAddressInfo($lng, $lat, $type)
    {
        $url = 'https://apis.map.qq.com/ws/geocoder/v1/?location=' . $lat . ',' . $lng . '&key=OV7BZ-ZT3HP-6W3DE-LKHM3-RSYRV-ULFZV';
        $client = new Client();
        ...
        if ($res->getStatusCode() != 200) {
            $errText = $type == 'sender' ? '商家发货地址异常' : '用户收货地址异常';
            throw new \Exception($errText);
        }

        $data = json_decode($res->getBody(), true);
        if (!isset($data['result']['address_component']['city'])) {
            $errText = $type == 'sender' ? '商家发货地址异常,城市不存在' : '用户收货地址异常,城市不存在';
            throw new \Exception($errText);
        }
