...
abstract class BaseSend extends Model
{
    ...
    public $update_send = false; // 订单是否完成发货
    ...
    public function checkOrder($order)
    {
        ...
        if (!$this->express_id) {
            if ($order->is_send == 1 && $order->sign != 'weekly_buy') {
                throw new \Exception('express_id参数异常');
            }

            $relation = $order->detailExpressRelation;
            if (count($relation) >= count($order->detail) && $order->sign != 'weekly_buy') {
            ...
            if ($count && $order->sign != 'weekly_buy') {
    ...
    public function saveOrderDetailExpress($order)
    {
        ...
        $this->update_send = count($order->detail) == $relationCount && $order->is_send == 0;

        if ($order->sign) {
            // 注：插件中可能修改update_send的状态
            $this->order = $order;
            $this->orderDetailExpress = $orderDetailExpress;
            try {
                $plugin = \Yii::$app->plugin->getPlugin($order->sign);
                $plugin->sendOrder($this);
