...
class ExpressSendForm extends BaseSend
{
    ...
    public function saveExtraData($orderDetailExpress)
    {
        ...
        if ($expressSingle) {
            $expressSingleOrder = \yii\helpers\BaseJson::decode($expressSingle->order);
            if (isset($expressSingleOrder['LogisticCode'])) {
                //快递鸟
                if ($expressSingleOrder['LogisticCode'] == $this->express_no) {
                    $orderDetailExpress->express_single_id = $this->express_single_id;
                }
            } else if ($expressSingleOrder['data'] && $expressSingleOrder['data']['kuaidinum']) {
                //快递100
                if ($expressSingleOrder['data']['kuaidinum'] == $this->express_no) {
                    $orderDetailExpress->express_single_id = $this->express_single_id;
                }
            }
        }
