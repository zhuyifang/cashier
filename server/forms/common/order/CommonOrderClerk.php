...
class CommonOrderClerk extends Model
{
    ...
    public function checkOrder()
    {
        ...
        if ($this->order->sign && $this->order->sign != $this->plugin) {
            \Yii::error('参数错误，请确认参数' . $this->plugin);
        }
        return $this->checkUserClerk();
    }

    public function checkUserClerk()
    {
        $clerkUser = $this->order->checkClerkPermission($this->order->store_id, $this->clerk_id, $this->order->mch_id);

        if (!$clerkUser) {
            throw new \Exception('没有核销权限，禁止核销');
        }
        return $this->clerk($clerkUser);
    }

    public function clerk($clerkUser)
    {
        $this->order->is_send = 1;
        $this->order->send_time = date('Y-m-d H:i:s', time());
        $this->order->is_confirm = 1;
        $this->order->confirm_time = date('Y-m-d H:i:s', time());
        $this->order->clerk_id = $clerkUser->id;
        $res = $this->order->save();

        if (!$res) {
            throw new \Exception($this->getErrorMsg($this->order));
        }
    ...
    {
        $orderClerk = OrderClerk::find()->where(['order_id' => $this->order->id])->one();
        if (!$orderClerk) {
            $orderClerk = new OrderClerk();
            $orderClerk->mall_id = \Yii::$app->mall->id;
            $orderClerk->affirm_pay_type = $this->action_type;
