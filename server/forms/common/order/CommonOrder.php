    ...
    public function confirm($order)
    {
        ...
        try {
            ...
            if ($order->pay_type == 2 && $order->is_pay == 0) {
                ...
                $exists = PaymentOrder::find()->where([
                    'payment_order_union_id' => $paymentOrder->payment_order_union_id, 'is_pay' => 0
                ])->andWhere(['!=', 'id', $paymentOrder->id])->exists();
                if (!$exists) {
                    $paymentOrder->paymentOrderUnion->is_pay = 1;
                    if (!$paymentOrder->paymentOrderUnion->save()) {
                        throw new \Exception($this->getErrorMsg($paymentOrder->paymentOrderUnion));
                    }
                }
            }
            $order->is_confirm = 1;
