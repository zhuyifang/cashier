...
class BasicForm extends Model
{
    ...
    public function rules()
    {
        return [
            [['level', 'is_rebate', 'price_type', 'share_condition', 'condition', 'pay_type', 'cash_max_day',
                'min_money', 'cash_service_charge', 'new_become_condition'], 'required'],
            [['level', 'is_rebate', 'price_type', 'share_condition', 'condition',
                'share_goods_status', 'is_show_share_level', 'form_status',
                'consume_condition', 'is_condition'], 'integer'],
            ...
            [['share_goods_warehouse_id'], function ($attr, $params) {
                if (
                    $this->share_goods_status == 2 && !($this->$attr && !empty($this->$attr))
                    && in_array('buy_goods', $this->new_become_condition)
                ) {
            ...
            [['cat_list'], function ($attr, $params) {
                if (
                    $this->share_goods_status == 3 && !($this->$attr && !empty($this->$attr))
                    && in_array('buy_goods', $this->new_become_condition)
                ) {
                    $this->addError($attr, $this->getAttributeLabel($attr) . '必须选择');
                }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'new_become_condition' => '成为分销商的条件',
            'is_condition' => '成为分销商是否需要条件',
        ];
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            if ($this->is_condition == 1 && (!$this->new_become_condition || empty($this->new_become_condition))) {
                throw new \Exception('成为分销商有条件时，必须选择一个条件');
            }
            foreach ($this->attributes as $index => $item) {
                if ($index == 'new_become_condition') {
                    $item = implode(',', $item);
                }
                $setList[] = [
                    'key' => $index,
