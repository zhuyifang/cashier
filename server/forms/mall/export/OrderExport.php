...
class OrderExport extends BaseExport
{
    ...
    {
        $query = $this->query;
        $query->with(['user.userInfo', 'clerk', 'store', 'detail.goods.goodsWarehouse', 'refund', 'paymentOrder.paymentOrderUnion', 'detail.expressRelation.orderExpress', 'detailExpress', 'detail.refund', 'detail.share'])
    ...
    protected function transform($list)
    {
        ...
        foreach ($this->fieldsKeyList as $item) {
            if (in_array($item, ['goods_name', 'attr', 'goods_num', 'goods_no', 'cost_price', 'send_type', 'express', 'express_no', 'city_name', 'city_mobile', 'rebate', 'first_user', 'second_user', 'third_user', 'first', 'second', 'third'])) {
                $sign = true;
                break;
            }
        }
            ...
            try {
                ...
                if ($sign) {
                    ...
                    foreach ($item->detail as $index => $detailItem) {
                        ...
                        $newArr['goods_name'] = isset($goodsInfo['goods_attr']['name']) ? $goodsInfo['goods_attr']['name'] : '';
                        $newArr['goods_num'] = intval($detailItem->num);
                        $costPrice = isset($goodsInfo['goods_attr']['cost_price']) ? $goodsInfo['goods_attr']['cost_price'] : 0;
                        ...
                        $newArr['price'] = isset($goodsInfo['goods_attr']['price']) ? (float)$goodsInfo['goods_attr']['price'] : 0;

                        $newArr['attr'] = $attr;
                        $newArr['goods_no'] = isset($goodsInfo['goods_attr']['no']) ? $goodsInfo['goods_attr']['no'] : '';
                        $newArr['bar_code'] = isset($goodsInfo['goods_attr']['bar_code']) ? $goodsInfo['goods_attr']['bar_code'] : '';
                        ...
                        $newItem = array_merge($newItem, $this->getShareData($detailItem));
                        $newList[] = $newItem;
                    }
                } else {
                    $refundPrice = 0;
    ...
    private function getShareData($orderDetail)
    {
        $share = $orderDetail->share;
        $data = [
            'rebate' => 0,
            'first_user' => '',
            'second_user' => '',
        ...
        if ($share) {
            $userIds = [$share->first_parent_id, $share->second_parent_id, $share->third_parent_id];

            $users = User::find()->andWhere(['id' => $userIds])->select(['id', 'username', 'nickname', 'mobile'])->with('share')->all();
            $newList[0] = '';
            foreach ($users as $user) {
                $shareName = $user->share ? $user->share->name : '';
                $shareMobile = $user->share ? $user->share->mobile : '';
                $newList[$user->id] = sprintf('id:%s 昵称:%s 姓名:%s 手机号:%s', $user->id, $user->nickname, $shareName, $shareMobile);
            }

            $isZigou = $share->user_id == $share->first_parent_id ? true : false;
            $data['rebate'] = $isZigou ? (float)$share->first_price : 0;
            $data['first'] = $isZigou ? (float)$share->second_price : (float)$share->first_price;
            $data['second'] = $isZigou ? (float)$share->third_price : (float)$share->second_price;
            $data['third'] = $isZigou ? 0 : (float)$share->third_price;
            $data['first_user'] = $isZigou ? $newList[$share->second_parent_id] : $newList[$share->first_parent_id];
            $data['second_user'] = $isZigou ? $newList[$share->third_parent_id] : $newList[$share->second_parent_id];
            $data['third_user'] = $isZigou ? '' : $newList[$share->third_parent_id];
        }
        return $data;
    }
