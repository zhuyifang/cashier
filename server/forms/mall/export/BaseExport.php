...
abstract class BaseExport extends Model
{
    ...
    public function exportAction($query, $parmas = [])
    {
        ...
        try {
            ...
            foreach ($query->asArray($isArray)->batch() as $item) {
                ...
                $this->handleExport($dataList, $fieldsNameList, $fileName, $id);
            ...
            // 如果总数为空 则导出空表
            if ($count == 0) {
                $this->handleExport([], $fieldsNameList, $fileName, $id);
            }
    ...
    public function handleExport($dataList, $fieldsNameList, $fileName, $id)
    {
