...
class MallGoodsExport extends BaseExport
{
    ...
    public function export($query = null)
    {
        ...
        try {
            ...
            foreach ($query->batch(300) as $index => $item) {
                // 临时 唯一文件名称
                $temporaryFileName = sprintf('%s%s%s', $this->getFileName(), time() + $index, '.csv');
    ...
    protected function transform($list)
    {
        ...
        foreach ($list as $item) {
            ...
            $arr['id'] = $item->id;
            $arr['name'] = $item->name;
            $arr['subtitle'] = $item->goodsWarehouse->subtitle;
            $arr['keyword'] = $item->goodsWarehouse->keyword;
            ...
            $arr['type'] = $item->goodsWarehouse->type;
