...
class NavbarEditForm extends Model
{
    ...
    public function save()
    {
        try {
            $this->checkData();
            $res = CommonOption::set(Option::NAME_NAVBAR, $this->data, \Yii::$app->mall->id, Option::GROUP_APP, \Yii::$app->user->identity->mch_id);
    ...
    public function checkData()
    {
        ...
        foreach ($this->data as $key => $item) {
            ...
            if (\Yii::$app->user->identity->mch_id) {
                if ($key == 'navs') {
                    foreach ($item as $item2) {
                        if (mb_strlen($item2['text']) > 5) {
                            throw new Exception('标签名称不能大于5个字符');
                        }
                    }
                }
            }
