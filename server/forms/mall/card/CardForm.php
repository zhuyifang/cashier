...
class CardForm extends Model
{
    ...
    {
        $detail = GoodsCards::find()->where(['mall_id' => \Yii::$app->mall->id, 'id' => $this->id])->with('storeRelation')->one();
        $newDetail = ArrayHelper::toArray($detail);
        $sign = '0000-00-00 00:00:00';
        if ($newDetail['begin_time'] == $sign || $newDetail['end_time'] == $sign) {
            $newDetail['time'] = [];
        } else {
            $newDetail['time'] = [$newDetail['begin_time'], $newDetail['end_time']];
        }
        ...
        $list = [];
        if ($detail['use_type'] == 1) {
            foreach ($detail->storeRelation as $item) {
                $list[] = [
                    'id' => $item->store->id,
