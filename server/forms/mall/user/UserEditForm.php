...
class UserEditForm extends Model
{
            ...
            if ($memberChange) {
                try {
                    ...
                    if ($this->member_level > 0) {
                        $mallMember = MallMembers::findOne([
                            'level' => $this->member_level, 'mall_id' => \Yii::$app->mall->id, 'is_delete' => 0
                        ]);
                        $name = $mallMember->name;
                    }
                    $this->sendTemplate($form, '会员等级变更:你的会员等级身份已变更为' . $name);
                    $this->sendSmsToUser($form, '会员等级', $name);
    ...
    public function updateUserLevel()
    {
        ...
        try {
            ...
            $memberChange = $user->identity->member_level != $this->member_level;
            ...
            if ($memberChange) {
                try {
                    ...
                    $this->sendTemplate($user, '会员等级变更:你的会员等级身份已变更为' . $name);
                    $this->sendSmsToUser($user, '会员等级', $name);
                } catch (\Exception $exception) {
                    \Yii::error("发送解除会员等级模板消息失败");
                    \Yii::error($exception);
                }
