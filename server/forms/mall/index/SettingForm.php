...
class SettingForm extends Model
{
    ...
    public $give_type;
    public $send_bg;
    public $send_text_color;
    public $send_num_type;
    public $send_num_money_max;
    public $send_num_overlapping;
    public $send_num;
    public $send_limit;
    public $has_rubik;
    public $rubik_data;
    public $is_show_parent_name;
    ...
    public function rules()
    {
        return [
            ...
            [['contact_tel', 'kdniao_mch_id', 'kdniao_api_key', 'member_integral_rule',
                ...
                'kdniao_select_type', 'give_type', 'send_bg', 'send_text_color', 'send_num_type', 'send_num_money_max'], 'string'],
            [['over_time', 'delivery_time', 'after_sale_time', 'member_integral',
                ...
                'is_goods_video', 'is_show_stock', 'is_use_stock', 'is_required_position', 'is_share_tip',
                'is_quick_customize', 'is_must_login', 'is_show_hot_goods', 'is_open','open_type','is_auto_open',
                'has_order_evaluate', 'order_evaluate_day', 'is_verify_mobile', 'kd100_yum', 'has_send',
                'send_num_overlapping', 'send_num', 'send_limit', 'has_rubik', 'is_show_parent_name'], 'integer'],
            [['name'], 'required',],
            ...
            [['good_negotiable', 'payment_type', 'send_type', 'business_time_type_day', 'business_time_type_week',
                'auto_business_time', 'quick_customize_params', 'quick_customize_new_params', 'send_type_desc',
                'time_list', 'week_list', 'customer_service_list', 'rubik_data', 'quick_customize_link'], 'safe'],
            [['delivery_time', 'after_sale_time'], 'integer', 'min' => 0, 'max' => 30],
            [['over_time'], 'integer', 'min' => 0, 'max' => 999999],
        ...
        try {
            ...
            foreach ($list as $item) {
                ...
                if ($item->key == 'web_service_url') {
                    $newList[$item->key] = urldecode($item->value);
                }
            }
            ...
            foreach ($this->attributes as $key => $value) {
                ...
                if (isset($newList[$key])  && $newList[$key] != $value) {
    ...
    private function checkData()
    {
        ...
        if ($this->has_send) {
            if ($this->send_num_type == 'money') {
                if ($this->send_num_money_max <= 0) {
                    throw new \Exception('抽奖赠送次数金额必须大于0');
                }

                if ($this->send_num <= 0) {
                    throw new \Exception('抽奖赠送次数必须大于0');
                }
            }

            if ($this->send_num_type == 'order') {
                if ($this->send_num <= 0) {
                    throw new \Exception('抽奖赠送次数必须大于0');
                }
            }

            if ($this->send_limit <= 0) {
                throw new \Exception('抽奖赠送次数限制必须大于0');
            }
        }
    }
}
