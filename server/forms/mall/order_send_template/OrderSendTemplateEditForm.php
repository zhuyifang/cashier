...
class OrderSendTemplateEditForm extends Model
{
    ...
    private function saveFile()
    {
        ...
        if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64, $result)) {
            ...
            $filePath = '/uploads/send-template/' . date('Y') . date('m') . date('d') . '/';
            $newPath = $basePath . '/web' . $filePath;
            if (!file_exists($newPath)) {
                //检查是否有该文件夹，如果没有就创建，并给予最高权限
                mkdir($newPath, 0777, true);
            }
            $newFilePath = $filePath . time() . ".{$type}";
            if (file_put_contents($basePath . '/web' . $newFilePath, base64_decode(str_replace($result[1], '', $base64)))) {
                return \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . $newFilePath;
            } else {
                return false;
            }
        } else {
