    ...
    public function search()
    {
        ...
        if ($order['refund']) {
            ...
            foreach ($order['refund'] as $item) {
                if ($item['is_refund'] == 1) {
                    $refundPrice += $item['reality_refund_price'];
                }
            }
            $order['refund_text'] = '';
            if ($order['new_status'] & 2) {
                $order['refund_text'] = sprintf('中断交易实际退款%.2f元', $refundPrice);
            } else {
                $order['refund_text'] = sprintf('售后退款%.2f元', $refundPrice);
            }
        ...
        $order['send_template_discount_price'] = price_format($order['total_goods_original_price'] + $order['express_price']- $order['total_pay_price']);
        ...
        $order = array_merge($order, CommonCustomization::getCommon()->getBtnName($order['detail']));
        $order['customization_status'] = CommonCustomization::getCommon()->getIsCustomization($order['detail']);
