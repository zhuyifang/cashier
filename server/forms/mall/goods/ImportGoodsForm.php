...
class ImportGoodsForm extends Model
{
    ...
    public function save()
    {
        ...
        try {
            ...
            if (count($errorList) > 0 || $this->current_num <= 1) {
                ...
                $option = CommonOption::set(Option::NAME_IMPORT_ERROR_LOG, $newArr, \Yii::$app->mall->id, Option::GROUP_ADMIN, \Yii::$app->user->identity->mch_id);
            }
    ...
    public function read_csv($file)
    {
        try {
            ...
            foreach ($list as $key => $value) {
                ...
                $arr['subtitle'] = $this->getNewData($value['副标题']);
                $arr['keyword'] = $this->getNewData($value['关键字']);
                ...
                $arr['type'] = $this->getNewData($value['商品类型']);
    ...
    private function getNewData($string)
    {
        // 处理编码问题
    ...
    private function saveGoods($data)
    {
        ...
        try {
            ...
            $goodsWarehouse->subtitle = $data['subtitle'];
            $goodsWarehouse->keyword = $data['keyword'];
            ...
            $goodsWarehouse->type = $data['type'];
            if (!$goodsWarehouse->save()) {
                throw new \Exception($this->getErrorMsg($goodsWarehouse));
            }

            $goods = new Goods();
            $groups = $goods->handleAttrGroups($data['attrGroups']);
            $attr = $goods->handleAttr($data['attr']);
            ...
            $goods->attr_groups = json_encode($groups);
            ...
            foreach ($attr as $attrItem) {
                ...
                // 记录规格ID数组
                $signIds = '';
                foreach ($attrItem['attr_list'] as $aLItem) {
            ...
            // 商品规格
            $keys = ['sign_id', 'stock', 'price', 'no', 'weight', 'pic_url', 'bar_code', 'cost_price', 'goods_id'];
