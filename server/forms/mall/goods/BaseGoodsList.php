...
abstract class BaseGoodsList extends Model
{
    ...
    public function getList()
    {
        ...
        try {
            ...
            // 插件拉取商城商品时 忽略定制商品
            if ($this->plugin) {
                $goodsWarehouseCondition[] = ['!=', 'type', 'form-goods'];
            }
            ...
            if (isset($search['keyword']) && $search['keyword'] && trim($search['keyword'])) {
                ...
                $goodsAttrGoodsIds = GoodsAttr::find()->andWhere(['is_delete' => 0])
                    ->andWhere(['no' => $keyword])->select('goods_id');
            ...
            $query = $this->timeSearch($query, $search);
    ...
    protected function handleData($list)
    {
        ...
        foreach ($list as $item) {
            ...
            foreach ($item->attr as $aItem) {
                ...
                $goodsPrice = $goodsPrice == 0 ? $aItem->price : min($goodsPrice, $aItem->price);
            }
