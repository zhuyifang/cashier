...
abstract class BaseBranch extends Model
{
    ...
    public function getSecondaryPermission($adminInfo)
    {
        ...
        try {
            ...
            if ($userGroup && $userGroup->permissionsGroup) {
                $groupPermissions = json_decode($userGroup->permissionsGroup->permissions, true);
                $groupMallPermissionFlip = array_flip($groupPermissions['mall_permissions']);
                $permission = $groupPermissions['secondary_permissions'];
                // 上传存储权限需先判断开关 再合并
                $permission['attachment'] = isset($groupMallPermissionFlip['attachment']) ? $permission['attachment'] : [];
            }
        }catch(\Exception $exception) {
            \Yii::error($exception);
        }
        ...
        if ($adminInfo->subjoin_permissions) {
            try {
                $subjoinPermissions = json_decode($adminInfo->subjoin_permissions, true);
                $permissionsFlip = $subjoinPermissions['mall'] ? array_flip($subjoinPermissions['mall']) : [];
                $secondary = $subjoinPermissions['secondary'];

                // 上传存储权限需先判断开关 再合并
                $secondary['attachment'] = isset($permissionsFlip['attachment']) ? $secondary['attachment'] : [];
                ...
                if ($secondary['template']['is_all']) {
                    ...
                    $permission['template']['list'] = array_values(array_column($permission['template']['list'], null, 'id'));
                }
                if ($secondary['template']['use_all']) {
                    ...
                    $permission['template']['use_list'] = array_values(array_column($permission['template']['use_list'], null, 'id'));
                }
            }catch(\Exception $exception) {
                \Yii::error($exception);
            }
    ...
    {
        return $user->mch_id == 0 && ($user->identity->is_operator == 0 && $user->identity->is_cashier == 0);
    }
