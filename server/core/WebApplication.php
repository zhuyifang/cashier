...
class WebApplication extends \yii\web\Application
{
    ...
    public $defaultDomain = 'DEFAULT_DOMAIN_NEW_MALL';
    ...
    public function url2str($data)
    {
        if (is_object($data)) {
            $data = (array)$data;
        }
        $replace = dirname(\Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl);
        $temp = str_replace($replace, \Yii::$app->defaultDomain, Json::encode($data));
        return Json::decode($temp, true);
    }
    ...
    public function str2url($data)
    {
        ...
        $replace = dirname($old);
        $temp = str_replace([\Yii::$app->defaultDomain, 'DEFAULT_DOMAIN_MALL'], [$replace, $old], Json::encode($data));
        return Json::decode($temp, true);
    }
}
