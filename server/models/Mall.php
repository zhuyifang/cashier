...
class Mall extends ModelActiveRecord
{
    ...
    public function getMallSetting($columns = [])
    {
        if ($this->mallAllOptions) {
            ...
            if (!isset($newOption['new_good_negotiable']) && isset($newOption['good_negotiable'])) {
                $newOption['new_good_negotiable'] = $newOption['good_negotiable'];
            }
            ...
            foreach ($defaultArr as $k => $item) {
                ...
                try {
                    ...
                    if (is_array($item)) {
                        $newOption[$k] = json_decode($newOption[$k], true);
                    }
                    ...
                    if ($k == 'quick_customize_link') {
                        $newOption[$k] = $newOption[$k] ? Json::decode($newOption[$k], true) : null;
                    }
    ...
    public function getDefault()
    {
        ...
        return [
            ...
            'has_send' => 0, //抽奖设置
            'give_type' => 'pond', //类型
            'send_bg' => $host  . "statics/img/mall/order/order-send_bg.gif",
            ...
            'negotiable_text' => '价格面议',
            'buy_goods_auth_link' => [],
            'buy_goods_auth_text' => '无权限购买',
            'is_show_parent_name' => 1,
            'new_customer' => []
        ];
    }
