...
class ShareSetting extends ModelActiveRecord
{
    ...
    const SHARE_CONDITION = 'share_condition'; // 成为分销商的方式
    ...
    const BECOME_CONDITION = 'become_condition'; // 成为分销商条件--已弃用改成new_become_condition
    ...
    const NEW_BECOME_CONDITION = 'new_become_condition'; // 成为分销商条件
    const IS_CONDITION = 'is_condition'; // 成为分销商是否需要条件
    ...
    public static function strToNumber($key, $str)
    {
        $default = ['level', 'is_rebate', 'price_type', 'share_condition', 'condition',
            ...
            'consume_condition', 'is_condition'];
    ...
    public static function getList($mallId)
    {
        ...
        // 将旧数据become_condition转化成新数据new_become_condition
        if (!isset($newList[self::NEW_BECOME_CONDITION])) {
            ...
            $newList[self::NEW_BECOME_CONDITION] = self::changeBecomeCondition($newList[self::BECOME_CONDITION]);
        } else {
            $newList[self::NEW_BECOME_CONDITION] = explode(',', $newList[self::NEW_BECOME_CONDITION]);
        }
        ...
        // 将旧数据become_condition拆分成两种状态0--无条件 1--有条件
        if (!isset($newList[self::IS_CONDITION])) {
            $newList[self::IS_CONDITION] = self::BECOME_CONDITION == 3 ? 0 : 1;
        }
        self::$setting = $newList;
        return $newList;
    }
    ...
    public static function changeBecomeCondition($becomeCondition)
    {
        switch ($becomeCondition) {
            case 1:
                $res = ['one_consume'];
                break;
            case 2:
