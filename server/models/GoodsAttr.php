...
class GoodsAttr extends ModelActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['sign_id', 'no', 'pic_url', 'bar_code'], 'string', 'max' => 255],
        ];
    }
     ...
     * @param string $sign string|null 订单类型
     * @return bool
     * @throws Exception
     */
    public function updateStock($num, $type, $goodsAttrId = null, $goodsAttrSign = null, $sign = '')
    {
        if ($goodsAttrId) {
        ...
        if ($type === 'add') {
            ...
            if ($num > $goodsAttr->stock && $sign !== 'teller') {
                throw new Exception('库存不足,$sign='.$sign);
            }
            $goodsAttr->stock -= $num;
            $goods->goods_stock -= $num;
        } else {
