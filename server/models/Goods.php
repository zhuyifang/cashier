 ...
 * @property int $is_shipping 是否包邮0--不包邮 1--包邮
 ...
 * @property int $is_share 是否允许分销0--不允许 1--允许
 ...
 * @property int $limit_buy_order 单笔订单可购买数量
 *
...
class Goods extends ModelActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['mall_id', 'mch_id', 'goods_warehouse_id', 'status', 'use_attr', 'goods_stock', 'virtual_sales',
                ...
                'is_setting_show_and_buy_auth', 'is_setting_send_type', 'is_time', 'limit_buy_order',
                'is_shipping', 'is_share'], 'integer'],
    ...
    public function getFormGoods()
    {
        return $this->hasOne(FormGoods::className(), ['goods_id' => 'id']);
    }

    public function handleAttrGroups($attrGroups)
    {
        return $this->addAttrGroupsId($attrGroups);
    }

    public function handleAttr($attr)
    {
        foreach ($attr as &$item) {
            foreach ($item['attr_list'] as &$alItem) {
                $alItem['attr_group_id'] = $this->newAttrGroupList[$alItem['attr_group_name']];
                $aKey = $alItem['attr_group_name'] . '_' . $alItem['attr_name'];
                $alItem['attr_id'] = $this->newAttrList[$aKey];
            }
            unset($alItem);
        }
        unset($item);
    ...
    public function addAttrGroupsId($list, &$id = 1, $group = [])
    {
        $newId = 1;
        foreach ($list as $key => $item) {
            if (isset($item['attr_list'])) {
                ...
                $newItemList = $this->addAttrGroupsId($item['attr_list'], $id, $item);
                $list[$key]['attr_list'] = $newItemList;
            } else {
                $aKey = $group['attr_group_name'] . '_' . $item['attr_name'];
                if (isset($this->signArr[$aKey])) {
                    $this->newAttrList[$aKey] = $this->signArr[$aKey];
                    $list[$key]['attr_id'] = $this->signArr[$aKey];
                } else {
                    $this->signArr[$aKey] = $id;
                    $this->newAttrList[$aKey] = $id;
                    $list[$key]['attr_id'] = $id++;
                }
            }
        }
