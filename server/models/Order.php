 ...
 * @property int $new_status 状态字段（转成二进制查看）从右往左查看依次：是否中断交易、是否关闭 00—未中断交易 11—中断交易，已关闭 10—已关闭
...
class Order extends ModelActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['mall_id', 'user_id', 'mch_id', 'use_user_coupon_id', 'use_integral_num', 'is_pay', 'pay_type',
                ...
                'clerk_id', 'store_id', 'is_comment', 'sale_status', 'status', 'distance', 'new_status'], 'integer'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'city_info' => '配送信息',
            'platform' => '平台信息',
            'new_status' => '状态字段（转成二进制查看）从右往左查看依次：是否中断交易、是否关闭
0(00)—未中断交易
3(11)—中断交易，已关闭
2(10)—已关闭',
        ];
        ...
        if ($order['detailExpress']) {
            foreach ($order['detailExpress'] as $deItem) {
                if ($deItem['expressSingle']) {
                    ...
                    $order = \yii\helpers\BaseJson::decode($deItem['expressSingle']['order']);
                    $newItem['print_teplate'] = isset($order['data']['imgBase64']) ? 'data:image/jpg;base64,' . current(json_decode($order['data']['imgBase64'],true)) : $deItem['expressSingle']['print_teplate'];
    ...
    public function getPriceList($order)
    {
        $list[] = ['label' => '商品总价', 'value' => sprintf('%s元', $order->total_goods_original_price)];
        if ($order->express_price > 0) {
            $list[] = ['label' => '运费', 'value' => sprintf('%s元', $order->express_price)];
        }

        if ($order->member_discount_price != 0) {
            $memberDiscountPrice = $order->member_discount_price > 0 ? sprintf('-%s元', $order->member_discount_price) : sprintf('+%s元', $order->member_discount_price);
            $list[] = ['label' => '会员优惠', 'value' => $memberDiscountPrice];
        }

        if ($order->back_price != 0) {
            $backPrice = $order->back_price > 0 ? sprintf('-%s元', $order->back_price) : sprintf('+%s元', $order->back_price);
            $list[] = ['label' => '订单改价', 'value' => $backPrice];
        }

        if ($order->express_original_price != $order->express_price) {
            if ($order->express_original_price > $order->express_price) {
                $expressPrice =price_format($order->express_original_price - $order->express_price);
                $list[] = ['label' => '运费改价', 'value' => sprintf('-%s', abs($expressPrice))];
            } else {
                $expressPrice = price_format($order->express_original_price - $order->express_price);
                $list[] = ['label' => '运费改价', 'value' => sprintf('+%s', abs($expressPrice))];
            }
        ...
        if ($order->integral_deduction_price > 0) {
            $list[] = ['label' => '积分抵扣', 'value' => sprintf('-%s元', $order->integral_deduction_price)];
        }

        if ($order->coupon_discount_price > 0) {
            $list[] = ['label' => '优惠券抵扣', 'value' => sprintf('-%s元', $order->coupon_discount_price)];
        }

        if ($order->full_reduce_price > 0) {
            $list[] = ['label' => '满减抵扣', 'value' => sprintf('-%s元', $order->full_reduce_price)];
        }

        $pluginData = $this->getPluginData($order, []);
        if ($pluginData && $pluginData['discount_list']) {
            foreach ($pluginData['discount_list'] as $item) {
                if ($item['value'] > 0) {
                    $list[] = ['label' => $item['label'], 'value' => sprintf('-%s元', $item['value'])];
                } else {
                    $list[] = ['label' => $item['label'], 'value' => sprintf('+%s元', $item['value'])];
                }
            }
        ...
        $list[] = ['label' => '实际支付', 'value' => sprintf('%s元', $order->total_pay_price)];
        return $list;
    }
    ...
    public static function checkOrder($order)
    {
        ...
        if ($order->new_status & 2) {
            throw new \Exception('订单已关闭，不可操作');
        }
        return $order;
    }
