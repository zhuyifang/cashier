...
//                    $query->andWhere(['status' => 1]);
    ...
    public function getGoods()
    {
        ...
        foreach ($list as $item) {
            ...
            foreach ($newItem['attr'] as &$attr) {
                $extra_attr = CommunityGoodsAttr::findOne(['goods_id' => $attr['goods_id'], 'attr_id' => $attr['id'], 'is_delete' => 0]);
                $attr['supply_price'] = empty($extra_attr) ? 0 : $extra_attr->supply_price;
            }
            unset($attr);
            $newItem['supply_price'] = ($newItem['use_attr'] == 1) ? 0 : $newItem['attr'][0]['supply_price'];
