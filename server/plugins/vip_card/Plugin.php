...
//            ],
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            if (is_array($typeInfo) && !empty($typeInfo)) {
                ...
                if ((($typeInfo['all'] == true) || $isInGoods || $isInCats) && $isAppoint && $isRule) {
                    ...
                    //折上折
                    $vipSubPrice = price_format($goodsItem['total_price'] * (1 - $vip->image_discount / 10));
                    if ($vipSubPrice != 0) {
                        // 减去超级会员卡优惠金额
    ...
    public function getRules($plugins = ['advance', 'booking', 'exchange', 'gift', 'miaosha', 'pintuan', 'flash_sale', 'wholesale', 'weekly_buy', 'diy'])
    {
        $list['rules'] = [''];
