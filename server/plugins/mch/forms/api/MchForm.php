...
use app\forms\common\platform\CashPayType;
...
class MchForm extends Model
{
    ...
    public function getDetail()
    {
        ...
        try {
            ...
            $favorite = Favorite::find()->andWhere(['mall_id' => \Yii::$app->mall->id, 'user_id' => \Yii::$app->user->id, 'favorite_mch_id' => $this->id, 'is_delete' => 0])->one();
            $count = Favorite::find()->andWhere(['favorite_mch_id' => $this->id, 'is_delete' => 0])->count();
            $newDetail['is_favorite'] = $favorite ? 1 : 0;
            $newDetail['favorite_count'] = $count;

            $navbar = CommonOption::get(
                Option::NAME_NAVBAR,
                \Yii::$app->mall->id,
                Option::GROUP_APP,
                (new NavbarForm())->getMchDefault(),
                $this->id
            );
                ...
                if (!\Yii::$app->user->isGuest) {
                    $cashList = MchCash::find()->alias('a')->select('a.type,a.type_data,a.use_qrcode')
                        ->where([
                            ...
                            'a.is_delete' => 0,
                            'a.mch_id' => $this->mch_id
                        ])->andWhere(['a.type' => ['wechat', 'alipay', 'bank']])
                        ->leftJoin(
                            ['b' => MchCash::tableName()],
                            'a.type=b.type and a.id < b.id and b.mall_id = :mall_id and b.mch_id = :mch_id',
                            [':mall_id' => \Yii::$app->mall->id, ':mch_id' => $this->mch_id]
                        )
                    ...
                    foreach ($cashList as $item) {
                        ...
                        $res['last_cash_list'][$item['type']] = $item;
                    }
                }
                $res = CashPayType::change($res, 'cash_type');
            }
