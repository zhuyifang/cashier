...
class CommonGoods extends Model
{
    ...
    public function getDiyGoods($array)
    {
        ...
        $goodsId = Goods::find()->where(['status' => 1, 'mall_id' => \Yii::$app->mall->id, 'is_delete' => 0])
            ...
            ->select('id');
        $goodsList = IntegralMallGoods::find()->with(['goods.goodsWarehouse', 'attr.attr'])
        ...
        foreach ($goodsList as $integralMallGoods) {
            ...
            $newItem = array_merge($newItem, $this->findMinIntegral($integralMallGoods));
            $newList[] = $newItem;
        }
        return [
            'list' => $newList,
    ...
    /**
     * 查找最小价格的最小积分
     * @param IntegralMallGoods $integralMallGoods
     * @return array
     */
    public function findMinIntegral($integralMallGoods)
    {
        $priceList = [];
        $integral = null;
        foreach ($integralMallGoods->attr as $value) {
            $priceList[] = $value->price;
            if ($value->price == min($priceList) && ($integral === null || $integral >= $value->attr->integral_num)) {
                $integral = $value->attr->integral_num;
            }
        }
        return [
            'price' => min($priceList),
