            ...
            switch ($coreTemplate->type) {
                case 'diy':
                    ...
                    foreach ($data as &$item) {
                        ...
                        if ($item['id'] == 'form-module' && isset($item['data']['list']) && is_array($item['data']['list'])) {
                            if (!isset($item['data']['form_info'])) {
                                throw new \Exception('请先到模板市场更新模板');
                            }
                            $formInfo = $item['data']['form_info'];
                            $diyForm = new DiyFormList();
                            $diyForm->attributes = $formInfo;
                            $diyForm->form_data = Json::encode($formInfo['form_data'], JSON_UNESCAPED_UNICODE);
                            $diyForm->is_delete = 0;
                            $diyForm->mall_id = \Yii::$app->mall->id;
                            if (!$diyForm->save()) {
                                throw new \Exception((new Model())->getErrorMsg($diyForm));
                            }
                            $item['data']['form_id'] = $diyForm->id;
                        }
                    }
                    unset($item);
