...
use app\forms\common\diy\PageValidateForm;
...
class TemplateEditForm extends Model
{
    ...
    public function get()
    {
        ...
        if ($template = $diyPage->templateOne ?? null) {
            ...
            foreach ($templateData as $datum) {
                ...
                if ($datum['id'] === 'form-module') {
                    $formId = $datum['data']['form_id'];
                    $diyFormList = DiyFormList::findOne($formId);
                    $datum['data']['list'] = \yii\helpers\BaseJson::decode($diyFormList['form_data']);
                }
    ...
    public function save()
    {
        ...
        try {
            ...
            $validate = new PageValidateForm();
            foreach ($data as $value) {
                if (!isset($value['id'])) {
                    continue;
                }
                $method = 'check' . hump($value['id'], '-');
