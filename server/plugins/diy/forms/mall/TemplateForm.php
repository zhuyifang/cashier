...
class TemplateForm extends Model
{
    ...
    {
        return [
            [['page', 'limit', 'id', 'is_home_page', 'is_link'], 'integer'],
            [['page'], 'default', 'value' => 1],
            [['limit'], 'default', 'value' => 10],
            [['type', 'keyword', 'access_limit', 'link'], 'string'],
            [['type', 'keyword'], 'default', 'value' => ''],
            ['platform', 'safe'],
            [['is_link'], 'default', 'value' => 0]
        ];
    }
    ...
    public function search()
    {
        ...
        foreach ($list as $key => $template) {
            ...
            $accessLimit = \yii\helpers\BaseJson::decode($template['access_limit']);
            array_push($newList, [
                'id' => $template['id'],
                'name' => $template['title'],
                ...
                'access_limit' => $accessLimit && !empty($accessLimit) ? $accessLimit : [
                        'is_all' => 1,
                        'member' => [],
                    ],
                'is_link' => intval($template['is_link']),
                'link' => \yii\helpers\BaseJson::decode($template['link']) ?? [],
    ...
    public function changeAccessLimit()
    {
        ...
        try {
            ...
            $diyPage->is_link = $this->is_link;
            $diyPage->link = $this->link;
            $diyPage->save();
            return [
                'code' => ApiCode::CODE_SUCCESS,
                'msg' => '保存成功',
