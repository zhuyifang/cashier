...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <el-dialog title="设置浏览权限" :visible.sync="browseVisible" class="browse-dialog" width="750px">
            <!-- 选择会员等级 -->
            ...
            <el-form @submit.native.prevent ref="browseForm" :model="browseForm" label-width="180px" size="small">
                ...
                <el-form-item label="无权限跳转链接" prop="is_link">
                    <el-switch v-model="browseForm.is_link" :active-value="1" :inactive-value="0"></el-switch>
                </el-form-item>
                <el-form-item label="无权限跳转链接设置" v-if="browseForm.is_link == 1">
<!--                    <el-input disabled style="max-width: 525px;width: 100%" size="small" v-model="browseForm.link.new_link_url">-->
                        <app-pick-link @selected="selectLink" :link="browseForm.link" :show-customer="false">
<!--                            <el-button size="mini">选择链接</el-button>-->
                        </app-pick-link>
<!--                    </el-input>-->
                    <div style="width: 100%;color: #c9c9c9;">注：建议跳转链接给予用户相关权限，若无权限易造成页面跳转死循环</div>
                </el-form-item>
            </el-form>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            browseSubmit() {
                if(this.browseForm.is_link == 1) {
                    if(!this.browseForm.link || !this.browseForm.link.new_link_url) {
                        this.$message.error('请选择跳转链接');
                        return false
                    }
                }else {
                    this.browseForm.link = {}
                }
                ...
                request({
                    ...
                    data: Object.assign({}, this.browseForm, {access_limit: JSON.stringify(this.browseForm.access_limit), link: JSON.stringify(this.browseForm.link)}),
                    method: 'post',
                }).then(e => {
            ...
            openBrowse(row) {
                this.browseForm = JSON.parse(JSON.stringify(row));
                if(this.browseForm.link) {
                    this.browseForm.link = !this.browseForm.link ? {} : this.browseForm.link
                }
