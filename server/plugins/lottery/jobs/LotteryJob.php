...
class LotteryJob extends BaseJob implements JobInterface
{
    ...
    public function checkPrizeTimeout()
    {
        ...
        if ($lottery) {
            //时间判断
            if (($lottery->is_trigger == 0 || $lottery->trigger_type === 'time')
                && $lottery->end_at > Date('Y-m-d H:i:s')
            ) {
                return false;
            }
            ...
            if ($lottery->status == 0) {
                $lottery->type = 3;
                $lottery->open_at = date('Y-m-d H:i:s');
                $lottery->save();
            ...
            //人数判断
            $people_num = $lottery->is_trigger == 1 ? $lottery->people_min_num : $lottery->join_min_num;

            if ($people_num > $count && $people_num > 0) {
                $lottery->type = 2;
                $lottery->open_at = date('Y-m-d H:i:s');
                $lottery->save();
