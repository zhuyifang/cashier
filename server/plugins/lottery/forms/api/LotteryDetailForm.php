    ...
    public function detail()
    {
        ...
        try {
            ...
            if ($lottery->log) {
                ...
                $transaction->commit();
                if ($lottery->is_trigger == 1
                    && $lottery->trigger_type === 'people'
                    && $lottery->people_min_num > 0
                ) {
                    $invite_allNum = LotteryLog::find()->where([
                        'mall_id' => \Yii::$app->mall->id,
                        'child_id' => 0,
                        'lottery_id' => $lottery->id
                    ])->count();
                    if ($lottery->people_min_num == $invite_allNum) {
                        \Yii::$app->queue->delay(0)->push(new LotteryJob([
                            'model' => $lottery,
                        ]));
                    }
                }
