...
class DelayForm extends Model
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $order = Order::findOne(['id' => $this->order_id, 'user_id' => \Yii::$app->user->id]);
            ...
            switch ($this->operate) {
                ...
                case 'cancel':
                    if ($express->is_delay == 0) {
                        throw new WeeklyBuyException('当前周期未顺延，不需要取消');
                    }
                    if (!($express->is_delay & 1)) {
                        throw new WeeklyBuyException('商户顺延，用户不可取消');
                    }
