...
//        }
    ...
    public function getGoodsExtra($goods)
    {
        ...
        if ($res['is_negotiable'] == 1) {
            $res['price_content'] = \Yii::$app->mall->getMallSettingOne('negotiable_text');
        }
        return $res;
    }
    ...
    public function getIsNegotiable($goods)
    {
        ...
        if ($setting['is_vip_show']) {
            ...
            if (!empty($vip_arr)) {
                if (!\Yii::$app->user->isGuest && in_array(\Yii::$app->user->identity->identity->member_level, $vip_arr)) {
                    $is_negotiable = 0;
                }
            } else {
                $is_negotiable = 0;
