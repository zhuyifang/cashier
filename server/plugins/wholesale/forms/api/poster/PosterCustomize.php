...
class PosterCustomize extends Model implements BaseConst
{
    ...
    public function traitPrice($model, $left, $top, $has_center, $color)
    {
        ...
        $isNegtiable = intval($setting['is_vip_show'] == 1 && (!in_array($level, $setting['vip_show_limit'])));

        if($isNegtiable == 1) {
            $text = $this->setText(\Yii::$app->mall->getMallSettingOne('negotiable_text'), $left, $top + 28, 52, $color);
            return [
                $text,
            ];
        } else {
