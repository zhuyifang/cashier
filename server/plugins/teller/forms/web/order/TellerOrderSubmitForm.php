...
use app\plugins\teller\forms\web\order\TellerOrderGoodsAttr;
...
class TellerOrderSubmitForm extends \app\forms\api\order\OrderSubmitForm
{
    public function setPluginData()
    {
        ...
        $this->setEnableFullReduce($tellerSetting['is_full_reduce'] > 0);
        $this->setEnableCoupon($tellerSetting['is_coupon'] > 0);
        $this->setEnableVipPrice($tellerSetting['svip_status'] > 0);
        $this->setEnableMemberPrice($tellerSetting['is_member_price'] > 0);
        $this->setEnableIntegral($tellerSetting['is_integral'] > 0);
    ...
    public function checkChangePrice($mchItem)
    {
        ...
        if ($tellerSetting['is_goods_change_price_type'] == 1) {
            ...
            if (isset($this->form_data['change_price_type'])) {
                // 最多可加
    ...
    protected function getGoodsItemData($item)
    {
        ...
        // 单个商品改价优惠
        $data['goods_change_price'] = 0;
        if (isset($item['new_unit_price']) && $item['new_unit_price']) {
            // 商品支持自定义修改单价
            if ($item['new_unit_price'] < 0) {
                throw new OrderException('商品自定义价格不能小于0');
            }
            $data['goods_change_price'] = price_format($data['goods_attr']['original_price'] - $item['new_unit_price']);

            $data['unit_price'] = $item['new_unit_price'];
            $data['total_original_price'] = price_format($item['new_unit_price'] * $data['num']);
            $data['total_price'] = price_format($item['new_unit_price'] * $data['num']);
            $data['is_level'] = 0;
            $data['is_level_alone'] = 0;
        }
    ...
    public function getAllData()
    {
        ...
        foreach ($data['mch_list'] as &$item) {
            ...
            foreach ($item['goods_list'] as &$goods) {
                ...
                // 商品改价总优惠
                $totalGoodsChangePrice = price_format($totalGoodsChangePrice + ($goods['goods_change_price'] * $goods['num']));
            }
            $item['total_discounts_price'] += $totalGoodsChangePrice;

            $item['total_goods_change_price'] = $totalGoodsChangePrice;
            $item['total_change_price'] = price_format($totalChangePrice);
            $item['change_price_type'] = isset($this->form_data['change_price_type']) ? $this->form_data['change_price_type'] : '';
