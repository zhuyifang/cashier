...
<div id="app" v-cloak>
		...
		<div v-loading="loading" flex-box="1" class="main-center" :class="activeTab == 1 || activeTab == 2 || addMoney || showHung || addCredit || changePrice || loginMember || step > 0 || goods || successPay ? 'padding':''">
			...
			<div v-else-if="showHung" class="choose-goods" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
				...
				<div class="choose-goods-info">
					<teller-order :time="lookTime" :stop="!showHung" :list="hungOrder" type="hung" @remark="addRemark" @click="toOrderDetail" @change="delHung" @add="addCount"></teller-order>
				</div>
			</div>
			<!-- 选择加钱 -->
				...
				<div class="choose-goods-info attr-view">
					...
					<div class="detail-goods-attr detail-goods-number" v-if="selectAttr.stock > 0" :style="{'bottom': view_bottom + 'px'}">
						...
						<div flex="dir:left cross:center">
							...
							<el-input @focus="inputStatus=true" @blur="inputStatus=false"size="small" type="number" oninput="this.value = this.value.replace(/[^0-9]/g, '');" :min="goods.min_number > 0 ? goods.min_number : 1" :max="selectAttr.stock" v-model="number"></el-input>
				...
				<div class="menu-goods-list">
					...
					<div class="menu-goods-item" flex="dir:left" v-for="(item,index) in count" :key="item.attr_id">
						...
						<div class="menu-goods-info dir-top-nowrap cross-center">
							...
							<div :style="{'visibility': temp && item.id == temp.id ? 'hidden' : 'visible','width': is_pad ? '80%': '50%'}" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
								<span style="margin-right: 5px" v-for="(attr,idx) in item.attr" :key="idx">{{attr.attr_group_name}}:{{attr.attr_name}}</span>
							</div>
							<div class="menu-goods-price" flex="dir:bottom" v-if="item.new_unit_price">
								<div>￥{{item.new_unit_price}}
									<span style="color: #999999;text-decoration: line-through;margin-left: 5px;">￥{{item.member_price ? item.member_price : item.selectAttr ? item.selectAttr.price : item.price}}
									</span>
								</div>
							</div>
							<div class="menu-goods-price" :style="{'height': item.member_price ? '37px' :'27px'}" flex="dir:bottom" v-else>
								<div>￥{{item.member_price ? item.member_price : item.selectAttr ? item.selectAttr.price : item.price}}</div>
								<div v-if="item.member_price" style="font-size: 12px;">会员价</div>
							</div>
							<div class="menu-goods-number" flex="dir:left cross:center">
								<img @click="changeUnitPrice(index)" src="./../plugins/teller/assets/img/change.png" alt="">
	...
	<el-dialog title="优惠明细" :visible.sync="discountVisible" width="320px">
		<div style="margin-top: -20px;font-size: 16px;" v-loading="countLoading">
			...
			<div style="margin-bottom: 5px;" v-if="+orderInfo.total_goods_change_price > 0 || +orderInfo.total_goods_change_price < 0" flex="main:justify">
				<div style="color: #6c6c6c;">商品改价</div>
				<div style="color: #ff4544">{{orderInfo.total_goods_change_price > 0 ? '-': '+'}}￥{{Math.abs(orderInfo.total_goods_change_price).toFixed(2)}}</div>
			</div>
	...
	<!-- 商品改价 -->
	<el-dialog title="修改单价" :visible.sync="changePriceVisible" width="600px">
		<el-input size="small" type="number" oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3');" :min="0" placeholder="请填写修改后价格" v-model="unitPrice">
		</el-input>
		<span slot="footer" class="dialog-footer">
			<el-button size="small" @click="changePriceVisible = false;unitPrice=''">取 消</el-button>
			<el-button size="small" type="primary" @click="changeGoodsPrice">确 定</el-button>
		</span>
	</el-dialog>
	<!-- 扫码后选择商品 -->
	<el-dialog title="选择商品" :visible.sync="codeGoodsVisible" width="980px">
		<div style="overflow: auto;" :style="{'max-height': screenHeight * 0.6 + 'px'}">
			<teller-goods name="full" :attr="true" :pad="is_pad" :list="code_goods_list" @click="addBarCodeGoods" :top="0"></teller-goods>
		</div>
	</el-dialog>
</div>
<script>
	const app = new Vue({
		...
		data() {
			...
			return {
				...
				changePriceVisible: false, // 修改商品单价弹窗
		...
		methods: {
			changeGoodsPrice() {
				this.count[this.changeUnitIndex].new_unit_price = Number(this.unitPrice).toFixed(2);
			...
			searchBarCode() {
				...
				request({
					...
					if (e.data.code === 0) {
						...
						if(e.data.data.goods.length == 1) {
							this.addBarCodeGoods(e.data.data.goods[0])
						}else {
							this.codeGoodsVisible = true;
							this.code_goods_list = e.data.data.goods;
						}
					} else {
			...
			addBarCodeGoods(goods) {
				this.codeGoodsVisible = false;
				this.code_goods_list = [];
				let same = false;
			...
			toggleView(name) {
				...
				if(name == 'addMoney') {
					...
					this.showHung = true;
					this.lookTime = (new Date()).valueOf();
				}
			},
			// 关闭成功提示
			closeSuccess() {
			...
			previewOrder() {
				...
				request({
					...
					if (e.data.code === 0) {
						...
						for(let item of this.orderInfo.goods_list) {
							for(let index in this.count) {
								if(item.goods_attr.id == this.count[index].goods_attr_id) {
									this.count[index].member_price = item.member_price
			...
			handleMenu(index,type) {
				...
				if(type == 'low') {
					if(this.count[index].num == this.count[index].min_number) {
						return false;
					}
					this.count[index].num--;
					this.count[index].price = +this.count[index].selectAttr.price* +this.count[index].num;
			...
			submitGoods() {
				...
				let min_number = this.goods.min_number > 0 ? this.goods.min_number - 1 : 0
				if(this.number > min_number) {
					...
					let para = {
						min_number: this.goods.min_number > 0 ? this.goods.min_number : 1,
			...
			handleGoods(type) {
				...
				if(type == 'low') {
					let min_number = this.goods.min_number > 0 ? this.goods.min_number : 1
					if(this.number == min_number) {
						return false;
					}
					this.number--;
			...
			toDetail(item) {
				...
				request({
					...
					if (e.data.code === 0) {
						...
						self.number = self.goods.min_number > 0 ? self.goods.min_number : 1;
			...
			getSetting() {
				...
				request({
					...
					if (e.data.code === 0) {
						...
						self.$message.error(e.data.msg);
						setTimeout(()=>{
                            console.error('javascript:history.go(-1)',e.data.msg)
