...
class BecomeCaptainJob extends BaseObject implements JobInterface
{
    ...
    public function end()
    {
        ...
        if ($this->flag == 1) {
            ...
            // 对这个用户上面的队长进行队长等级升级判断
            if ($relation && $parentCaptain) {
                $this->updateLevel($parentCaptain);
            }
        }
    }
    ...
    /**
     * 对这个用户上面的队长进行队长等级升级判断
     * @param BonusCaptain $captain
     * @retrun Bool
     */
    protected function updateLevel($captain)
    {
        if (!$captain) {
            return false;
        }
        \Yii::$app->trigger(BonusMembers::UPDATE_LEVEL, new MemberEvent([
            'captain' => $captain
        ]));
        // 判断当前队长是否存在上级
        if ($captain->userInfo->parent_id) {
            $parentCaptain = BonusCaptain::findOne([
                'user_id' => $captain->userInfo->parent_id, 'status' => 1, 'is_delete' => 0,
            ...
            if ($parentCaptain) {
                ...
                // 判断当前队长的上级是否是其他团队的队员
                $relation = BonusCaptainRelation::findOne([
                    'user_id' => $captain->userInfo->parent_id, 'is_delete' => 0
                ]);
                if ($relation) {
                    $parentCaptain = BonusCaptain::findOne([
                        'user_id' => $relation->captain_id, 'status' => 1, 'is_delete' => 0,
                        'mall_id' => $this->mall->id
                    ]);
                    $this->updateLevel($parentCaptain);
                }
