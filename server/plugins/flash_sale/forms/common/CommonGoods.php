...
class CommonGoods
{
    ...
    public static function getDiyGoods($array)
    {
        ...
        $goodsIdList = Goods::find()->where(['status' => 1, 'mall_id' => Yii::$app->mall->id, 'is_delete' => 0])
            ...
            ->select('id');
        $flashSaleGoodsList = FlashSaleGoods::find()->alias('p')->with(['goods.goodsWarehouse', 'activity'])
        ...
        foreach ($flashSaleGoodsList as $flashSaleGoods) {
            ...
            $newItem = array_merge(
                ...
                [
                    ...
                    'activity_name' => $flashSaleGoods->activity->title,
                    'start_at' => $flashSaleGoods->activity->start_at,
                    'end_at' => $flashSaleGoods->activity->end_at,
                ]
            );
            $newList[] = $newItem;
        }
