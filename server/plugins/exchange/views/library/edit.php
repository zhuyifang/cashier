    ...
    <el-dialog class="dialog-choose" :title="title" :visible.sync="listDialog" width="50%" :before-close="handleClose">
        ...
        <div class="dialog-choose-radio" v-else-if="chooseType == 'goods' && setGoods">
            ...
            <el-table :data="chooseGoods[0].attr" border style="width: 100%;margin-top: 30px">
                ...
                <el-table-column label="数量（注意：单个兑换码可兑换的奖品数量，不是码库总商品数！）" prop="number" width="300">
                    <template slot-scope="scope">
    ...
    <el-dialog title="批量生成兑换码" :visible.sync="appendDialog" width="30%">
        <el-form @submit.native.prevent size="small" label-width="100px">
            <el-form-item label="生成数量" prop="num">
                <el-input v-model="num" min="0" oninput="if(this.value.length>4)this.value=10000;this.value = this.value.replace(/[^0-9]/g, '')" type="number" :max="10000">
                    <template slot="append">条</template>
                </el-input>
                <div style="color: #999;font-size: 12px;">单次最多同时生成10000条</div>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
        ...
        methods: {
            ...
            append() {
                if(this.num > 0) {
                    ...
                    this.num = this.num > 10000 ? 10000 : this.num
                    request({
                        params: {
