...
class OrderPayEventHandler extends OrderPayedHandlerClass
{
    ...
    protected function pintuan()
    {
        ...
        if ($orderRelation->pintuanOrder->status == 0) {
            $delay = $orderRelation->pintuanOrder->pintuan_time * 60 * 60;
            $expectedOverTime = time() + $delay;
            $pintuanGoods = $orderRelation->pintuanOrder->goods->pintuanGoods;

            // 预计结束时间
            if ($pintuanGoods->end_time != '0000-00-00 00:00:00') {
                $endTime = strtotime($pintuanGoods->end_time);
                // 预计结束时间
                $expectedOverTime = $expectedOverTime > $endTime ? $endTime : $expectedOverTime;
                // 定时任务执行时间 如果商品下架时间早 需按下架时间来创建延时任务
                $delay = ($endTime - time()) > $delay ? $delay : $endTime - time();
            }

            $orderRelation->pintuanOrder->status = 1;
            $orderRelation->pintuanOrder->expected_over_time = $expectedOverTime;
