    ...
    public function poster()
    {
        ...
        try {
            if ($this->goods_id) {
                ...
                $data = $this->getGoodsPoster($goods, array_shift($share_pic), $list['total_sales']);
            } else {
                $goods_id = null;
                ...
                if ($model->goods_id) {
                    ...
                    $data = $this->getGoodsPoster($model->goods, array_shift($share_pic), $list['total_sales']);
                } else {
                    $share_pic = \yii\helpers\BaseJson::decode($model->share_pic);
                    $data = $this->getDynamic(array_shift($share_pic));
                }
            }
            return [
    ...
    public function getGoodsPoster($goods, $share_pic, $sales = 0)
    {
        ...
        if (isset($option['price'])) {
            ...
            if ($goods->mallGoods['is_negotiable']) {
                $price_str = \Yii::$app->mall->getMallSettingOne('negotiable_text');
