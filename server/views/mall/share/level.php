    ...
    <el-card shadow="never" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div class="table-body">
            ...
            <el-table
                ...
                <el-table-column prop="level" label="分销商等级" width="110">
                ...
                <el-table-column label="升级条件"
                    ...
                    <template slot-scope="scope">
                        ...
                        <template v-else>
                            <div v-for="(item,index) in scope.row.condition_list" :key="item.key" :style="{'margin-top': index > 0 ? '10px' : '0'}" v-show="index < 2">
                                <div flex="dir:left cross:center">
                                    <div style="width: 98px;text-align: right;">{{item.key}}</div>
                                    <div style="margin-left: 28px;color: #409EFF;">
                                        <app-ellipsis :line="1" v-if="item.key == '下线用户数' || item.key == '下线分销商人数'">满{{item.value}}人</app-ellipsis>
                                        <app-ellipsis :line="1" v-else>满{{item.value}}元</app-ellipsis>
                                    </div>
                                </div>
                            </div>
                            <div v-if="scope.row.condition_list.length > 2">
                                <el-popover
                                        placement="bottom"
                                        width="250"
                                        trigger="hover">
                                    <div flex="dir:left cross:center" v-for="(item,index) in scope.row.condition_list" :key="item.key" :style="{'margin-top': index > 0 ? '10px' : '0'}">
                                        <div style="width: 98px;text-align: right;flex-shrink: 0">{{item.key}}</div>
                                        <div style="margin-left: 28px;color: #409EFF;">
                                            <app-ellipsis :line="1" v-if="item.key == '下线用户数' || item.key == '下线分销商人数'">满{{item.value}}人</app-ellipsis>
                                            <app-ellipsis :line="1" v-else>满{{item.value}}元</app-ellipsis>
                                        </div>
                                    </div>
                                    <div slot="reference" flex="dir:right" style="width: 108px;margin-top: 4px;">
