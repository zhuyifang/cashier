...
Yii::$app->loadViewComponent('diy-form/goods-form');
...
<style>
    ...
    .app-order-list .goods-item .goods.other {
        min-height: 65px;
    }

    .app-order-list .goods-item .goods.other .el-button {
        height: 26px;
        line-height: 26px;
        padding: 0 10px;
        border-color: #409eff;
    ...
    .app-order-list .goods-item .goods.other .el-button:hover {
    ...
    .app-order-list .weekly-buy-info {
        margin-left: 86px;
    }

    .app-order-list .weekly-buy-info img {
        width: 16px;
        height: 16px;
...
<template id="app-order-list" ref="appOrder">
    <div class="app-order-list" style="margin-bottom: 20px;">
        ...
        <div class="table-body">
            <app-search
                    :is_show_form_goods="hide_function.is_show_form_goods"
                    @search="toSearch"
            ...
            <div v-loading="loading" v-if="list && list.length > 0">
                <el-card
                    ...
                    <div slot="header" class="app-order-head" flex="cross:center">
                        ...
                        <div flex="cross:center" class="remark-box"
                             v-if="item.remark || (item.merchant_remark_list && item.merchant_remark_list.length > 0)">
                        ...
                        <div class="app-order-offline" flex="dir:left wrap:wrap">
                            ...
                            <template v-else-if="isShowOrderStatus">
                                ...
                                <el-tag size="small" type="danger" v-if="item.new_status >  1">已关闭</el-tag>
                                <template v-else>
                                    ...
                                    <el-tag size="small" type="success"
                                            v-if="item.is_send == 1 && item.is_confirm == 0">
                                        已发货
                                    </el-tag>
                                    <el-tag size="small" type="success"
                                            v-if="item.is_confirm == 0 && item.is_send == 1">
                                        未收货
                                    </el-tag>
                        ...
                        <div class="weekly-buy-info"
                             v-if="item.sign == 'weekly_buy' && item.plugin_data && item.plugin_data.extra && item.plugin_data.extra.weekly_buy"
                             flex="dir:left cross:center">
                            <div>共{{item.plugin_data.extra.weekly_buy.week_number}}期</div>
                            <img v-if="item.plugin_data.extra.weekly_buy && ((item.plugin_data.extra.weekly_buy.express_list && item.plugin_data.extra.weekly_buy.express_list.length > 0) || item.is_confirm == 1) && item.new_status < 2 && item.cancel_status == 0"
                                 src="statics/img/mall/weekly-tips.png" alt="">
                            <div v-if="item.is_confirm == 1 && item.new_status < 2 && item.cancel_status == 0"
                                 style="color: #409EFF">已收货
                            </div>
                            <div v-else-if="item.plugin_data.extra.weekly_buy.express_list && item.plugin_data.extra.weekly_buy.express_list.length > 0 && item.new_status < 2 && item.cancel_status == 0"
                                 style="color: #409EFF">
                                第{{item.plugin_data.extra.weekly_buy.now_express.week_number}}期待{{item.plugin_data.extra.weekly_buy.now_express.is_send
                                == 0 ? '发货 ': '收货 '}}
                                {{item.plugin_data.extra.weekly_buy.now_express.send_time}}{{item.plugin_data.extra.weekly_buy.now_express.is_send
                                == 0 ? '' : '已'}}发货
                            </div>
                        </div>
                    ...
                    <div class="app-order-body">
                        ...
                        <div class="goods-item" :style="{width: orderTitle[0].width}">
                            <div class="goods" v-for="(goods, iIndex) in item.detail" v-show="iIndex < 10">
                                ...
                                <template v-if="goods.goods">
                                    <image style="width: 24px;height: 24px;position: absolute;left: 20px;z-index: 2;"
                                           v-if="goods.goods.goodsWarehouse.type === 'form-goods' && isGoodsType && hide_function.is_show_form_goods"
                                           src="statics/img/mall/form-goods.png"></image>
                                    <image style="width: 24px;height: 24px;position: absolute;left: 20px;z-index: 2;"
                                ...
                                <div flex="dir:left">
                                    <div class="goods-info">
                                        ...
                                        <div style="margin-bottom: 24px;">
                                            <span style="margin-right: 10px;flex-wrap: wrap" flex="dir:left cross:center">
                                                <slot name="attr" :item="item">
                                                    <span style="flex-grow: 0;flex-shrink: 0">规格：</span>
                                                    <el-tag size="mini"
                                                            style="margin-right: 5px;max-width: 210px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis"
                                            ...
                                            <div style="margin-top: 10px"
                                                 flex="dir:left cross:center"
                                                 v-if="item.customization_status == 1 || (item.select_date  && item.select_date.day)"
                                                 >
                                                <div style="margin-right: 30px" v-if="item.select_date  && item.select_date.day" flex="dir:left cross:center">
                                                    <div>预约时间：</div>
                                                    <div v-if="item.select_date.is_alone == 1"  style="color:#409EFF">{{item.select_date.new_before}}</div>
                                                    <div v-if="item.select_date.is_alone == 0" style="color:#409EFF">
                                                        {{item.select_date.new_before}}-{{item.select_date.new_after}}   共{{item.select_date.day}}{{item.select_date.place_unit}}
                                                    </div>
                                                </div>
                                                <goods-form v-if="item.customization_status == 1" :detail-id="goods.id">
                                                    <el-button style="padding:6px 16px; font-size: 12px;background:white;color: #409EFF !important;" marsize="mini" plain
                                                               type="primary">
                                                        {{item.btn_name}}
                                                    </el-button>
                                                </goods-form>
                            ...
                            <div v-if="item.detail.length > 10" class="goods other" flex="main:center cross:center">
                                <el-button plain type="primary" @click="toDetail(item.id, true)">其余还有{{item.detail.length - 10}}个商品，共{{item.detail.length}}个商品</el-button>
                            </div>
                        </div>
                        <div flex="cross:center" class="app-order-info" :style="{width:orderTitle[1].width}">
                            <div flex="dir:top">
                                ...
                                <span v-if="item.sign == 'community' && isShowProfit"
                                      style="color: #ff4544;margin-top: 5px;">(团长利润￥{{item.plugin_data.extra &&item.plugin_data.extra.profit_price ? item.plugin_data.extra.profit_price : '0.00'}})</span>
                            </div>
                        </div>
                        <div flex="cross:center" class="app-order-info" v-if="showMoreInfo"
                             :style="{width:orderTitle[2].width}">
                            <slot name="orderInfo" :order="item"></slot>
                        </div>
                        <div v-if="isShowAction" class="app-order-info"
                             :style="{width: showMoreInfo ? orderTitle[3].width : orderTitle[2].width}"
                            ...
                            <div flex="wrap:wrap cross:center">
                                ...
                                <el-tooltip class="item" effect="dark" content="确认收货" placement="top">
                                    <img class="app-order-icon" src="statics/img/mall/order/confirm.png" alt=""
                                         v-if="item.is_recycle == 0 && item.is_send == 1 && item.is_confirm == 0 && isShowConfirm && item.status != 0 && item.is_confirm_show && item.sign != 'weekly_buy' && item.new_status < 2"
                                         @click="confirm(item)">
                                </el-tooltip>
                                <el-tooltip class="item" effect="dark" content="确认收货" placement="top">
                                    <img class="app-order-icon" src="statics/img/mall/order/confirm.png" alt=""
                                         v-if="item.sign == 'weekly_buy' && item.is_recycle == 0 && item.is_send == 1 && item.is_confirm == 0 && isShowConfirm && item.status != 0 && item.send_type == 0 &&  item.plugin_data.extra.weekly_buy.now_express.is_send == 1 && item.plugin_data.extra.weekly_buy.now_express.is_confirm == 0 && item.new_status < 2"
                                         @click="confirm(item)">
                                </el-tooltip>
                                ...
                                <el-tooltip class="item" effect="dark" :content="item.is_pay == 1 ? '核销订单' : '确认收款'"
                                    ...
                                    <img class="app-order-icon" @click="openDialog(item, clerkVisible = true)"
                                         v-if="item.send_type == 1 && (item.is_pay == 1 || item.pay_type == 2) && item.clerk == null && item.is_send == 0 && item.is_clerk_show && item.is_recycle == 0 && isShowClerk && item.is_recycle == 0 && item.status != 0 && item.cancel_status != 1 && item.new_status < 2"
                                ...
                                <template>
                                    ...
                                    <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="app-order-icon" @click="openExpress(item,'send')"
                                             v-if="(item.action_status.is_express_send && isShowSend && item.sign != 'weekly_buy' && item.new_status < 2 && item.cancel_status == 0) || (item.sign == 'weekly_buy' && item.plugin_data && item.plugin_data.extra.weekly_buy && item.plugin_data.extra.weekly_buy.now_express && item.plugin_data.extra.weekly_buy.now_express.is_can_send && item.send_type == 0 && isShowSend && item.new_status < 2 && item.cancel_status == 0)"
                                             src="statics/img/mall/order/send.png" alt="">
                                    </el-tooltip>
                                    <!-- 同城配送发货 选择配送员 -->
                                    <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="app-order-icon" @click="openExpress(item,'send')"
                                             v-if="(item.action_status.is_city_send && isShowSend && item.new_status < 2 && item.cancel_status == 0) || (item.sign == 'weekly_buy' && item.plugin_data && item.plugin_data.extra && item.plugin_data.extra.weekly_buy && item.plugin_data.extra.weekly_buy.now_express && item.plugin_data.extra.weekly_buy.now_express.is_can_send && item.send_type == 2 && isShowSend && item.new_status < 2 && item.cancel_status == 0)"
                                             src="statics/img/mall/order/send.png" alt="">
                                    </el-tooltip>
                                    <!-- 到店自提也可发货 -->
                                    <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="app-order-icon" @click="storeOrderSend(item)"
                                             v-if="item.send_type == 1 && (item.is_pay == 1 || item.pay_type == 2) && item.is_send == 0 && item.cancel_status == 0 && item.is_send_show == 1 && isShowSend && item.is_recycle == 0 && item.status != 0 && item.new_status < 2"
                                             src="statics/img/mall/order/send.png" alt="">
                                    </el-tooltip>
                                </template>
                                <!-- 打印小票 -->
                                <el-tooltip class="item" effect="dark" content="打印小票" placement="top">
                                    <img class="app-order-icon"
                                         v-if="item.is_recycle == 0 && isShowPrint && item.new_status < 2"
                                         @click="showPrintDialog(item.id)"
                                         src="statics/img/mall/order/print.png" alt="">
                                </el-tooltip>
                                <!-- 打印发货单 -->
                                <el-tooltip class="item"
                                            v-if="item.action_status && item.action_status.is_print_send_template == 1 && item.new_status < 2"
                                ...
                                <!-- 修改快递单号 -->
                                <template
                                        v-if="item.detailExpress && item.detailExpress.length == 1 && item.is_send == 1 && item.cancel_status != 1 && item.is_confirm == 0 && item.is_recycle == 0 && isShowSend && item.status != 0 && item.is_update_send == 1 && item.sign != 'weekly_buy'">
                                    <el-tooltip class="item" effect="dark"
                                                :content="item.send_type == 2 ? '修改配送员' : '修改快递单号'" placement="top">
                                        <img class="app-order-icon"
                                ...
                                <!--多个物流信息订单 需到订单详情修改 -->
                                <template
                                        v-else-if="item.detailExpress && item.detailExpress.length >= 1 && item.cancel_status != 1 && item.is_confirm == 0 && item.is_recycle == 0 && isShowSend && item.status != 0 && item.is_update_send == 1 && item.sign != 'weekly_buy'">
                                    <el-tooltip class="item" effect="dark"
                                                :content="item.send_type == 2 ? '修改配送员' : '修改快递单号'" placement="top">
                                        <img class="app-order-icon"
                                ...
                                <el-tooltip class="item" effect="dark" content="查看订单详情" placement="top">
                                    <img v-if="isShowDetail" class="app-order-icon" @click="toDetail(item.id, true)"
                    ...
                    <div class="card-footer">
                        ...
                        <template v-if="item.send_type == 1">
                            ...
                            <div style="margin: 10px 0;">收货人: {{item.name}} 电话：{{item.mobile}}</div>
                            <div v-if="item.orderClerk && item.orderClerk.clerk_remark" style="margin: 10px 0;">核销备注:
                                {{item.orderClerk.clerk_remark}}
                            </div>
                        </template>
                        ...
                        <div class="seller-remark" v-if="item.seller_remark">商家备注：{{item.seller_remark ?
                            item.seller_remark : item.words}}
                        </div>
            ...
            <div v-for="(item) in printData"
                ...
                <div :style="{width: mmConversionPx(printPar.stencil_width) + 'px',minHeight:mmConversionPx(printPar.stencil_high) + 'px', cursor: 'pointer', border: `${mmConversionPx(printPar.border_width)}px solid #000000`, boxSizing: 'content-box', margin: 0}">
                    <div style="display: inline-block;position: relative;white-space: nowrap;width: 100%;height: 50px">
                        ...
                        <div v-show="!printPar.order.hidden_bar_code"
                             style="width: 42%;height: 50px;border-left: 1px solid #000000;padding: 8px 0;text-align: center; box-sizing: border-box;display: inline-block;position: absolute;right: 0;">
                    ...
                    <div>
                        <div style="box-sizing:border-box;width: 100%;display: flex;border-bottom:1px solid #000000;position: relative;left: -0.5px;"
                            ...
                            <div :style="tableWidth" style="display: flex;"
                                 v-if="printPar.goodsInf.name || printPar.goodsInf.attr || printPar.goodsInf.article_number">
                        ...
                        <div v-for="good in item.detail"
                            ...
                            <div :style="tableWidth" style="display: flex;"
                                ...
                                <div
                                        :style="{width: printPar.goodsInf.article_number ? '75%' :'100%'}"
                                        style="min-width: 0;flex-grow: 1; word-break: break-all;flex-shrink: 1;-webkit-box-flex: 1;word-wrap:break-word;box-sizing:border-box;word-wrap: break-word;border-left: 1px solid #000000;padding:  10px 10px 10px 0.5%;font-size:10px;position: relative;flex-shrink: 0"
                                        v-if="printPar.goodsInf.name || printPar.goodsInf.attr">
                                    {{good.name}}:{{good.attr}}
                                </div>
    ...
    Vue.component('app-order', {
        ...
        methods: {
            ...
            confirm(item) {
                this.$confirm('是否确认收货?', '提示', {
                    ...
                    if (item.sign == 'weekly_buy') {
                        para.week_number = item.plugin_data.extra.weekly_buy.now_express.week_number;
                    }
                    request({
                        params: {
                            r: item.sign == 'weekly_buy' ? 'plugin/weekly_buy/mall/order/confirm' : 'mall/order/confirm',
                        },
                        data: para,
                        method: 'post',
                    }).then(e => {
            ...
            printTeplate(htmlData) {
                if (htmlData.indexOf('data:image/jpg;base64,') === -1) {
                    myWindow = window.open('', '_blank');
                    myWindow.document.write(htmlData);
                    myWindow.focus();
                } else {
                    this.adminPrint('preview', htmlData);
                }
            },
            // 删除订单
            toDelete(e) {
            ...
            getList() {
                ...
                localStorage.setItem('ORDER_SEARCH', JSON.stringify(this.search));
                    ...
                    if (e.data.code === 0) {
                        ...
                        newList.forEach(item => {
                            ...
                            item.detail = item.detail.map(item2 => {
                                delete item2.goods_info.goods_attr.db_customization
                                delete item2.goods_info.goods_attr.date
                                return item2;
                            })
                        })
                ...
                if (this.isSendTemplate) {
                    ...
                    request({
                        ...
                        if (order_detail.length > 0) {
                            for (let index in order_detail) {
                                order_detail[index].id = +index + 1
                            }
                            let data = {
                                order_no: order.order_no,
                                pay_time: order.pay_time,
