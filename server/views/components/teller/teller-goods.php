...
<style>
    ...
    .teller-goods .teller-goods-item.border {
    ...
    .teller-goods .teller-goods-item .teller-goods-info .teller-goods-name.one {
        -webkit-line-clamp: 1;
        height: 21px;
        margin-top: -5px;
    }
...
<template id="teller-goods">
    <div class="teller-goods" :infinite-scroll-disabled="!list || list.length == 0" v-infinite-scroll="load" :class="pad ? 'pad': ''" flex="dir:left" :style="{'padding-top': paddingTop + 'px'}" v-if="list.length > 0">
        <div @click="toDetail(item)" class="teller-goods-item" :class="attr ? 'border' : ''" v-for="(item,index) in list" :key="index">
            ...
            <div class="teller-goods-info">
                <div class="teller-goods-name" :class="attr ? 'one' :''">{{item.name}}</div>
                <div style="color: #999999;font-size: 14px;margin-top: 5px;height: 35px" class="teller-goods-name" v-if="attr"><span v-for="attr in item.attr">{{attr.attr_group_name}}:{{attr.attr_name}}</span></div>
