    ...
    <div class="teller-order" v-infinite-scroll="load" :infinite-scroll-disabled="stop">
        <div v-for="(count,idx) in list" class="hung-list" :key="idx">
            ...
            <div style="margin-bottom: 4px;">
                订单总价: <span style="color: #ff4544;">￥{{type == 'order' ? count.total_pay_price : count.total}}</span>
                <span style="margin-left: 16px;color: #999999">等{{type == 'order' ? count.goods_count: count.list.length}}件商品</span>
                <span style="margin-left: 70px" v-if="time > 0 && count.timeStauts &&  count.timeStauts != undefined">挂单时间: <span style="color: #ff4544;">{{count.timeStauts}}</span></span></div>
            <div style="width: 60%;">
        ...
        created() {
            if(this.time > 0) {
                for(let item of this.list) {
                    let time = Math.floor((this.time - item.time) / 1000 / 60);
                    let hour = Math.floor(time / 60);
                    item.timeStauts = hour > 0 ? hour + '小时前' : time > 0 ? time + '分钟前' : '刚刚'
                }
            }
        },
        watch: {
            time: {
                handler(newValue) {
                    if(newValue > 0) {
                        for(let item of this.list) {
                            let time = Math.floor((newValue - item.time) / 1000 / 60);
                            let hour = Math.floor(time / 60);
                            item.timeStauts = hour > 0 ? hour + '小时前' : time > 0 ? time + '分钟前' : '刚刚'
                        }
                    }
