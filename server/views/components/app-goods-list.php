...
<style>
    ...
    .app-goods-list .is_mch .el-table__fixed-body-wrapper {
        top: 65px!important;
    }
</style>
<template id="app-goods-list">
    <div class="app-goods-list">
        <el-card v-loading="listLoading" class="box-card" shadow="never" style="border:0"
            ...
            <div class="table-body" :class="is_mch ? 'is_mch' : ''">
                <app-search :tabs="tabs" :place-holder="placeholder" :is-show-cat="isShowCat" :new-search="search" :is-goods-type="isGoodsType" :is_mch="is_mch" :is_show_form_goods="hide_function.is_show_form_goods" :is_show_ecard="hide_function.is_show_ecard" @to-search="toSearch"
                        ...
                        <template slot-scope="scope">
                            <div flex="box:first">
                                <div style="padding-right: 10px;">
                                    <image style="width: 24px;height: 24px;position: absolute;z-index: 2" v-if="scope.row.goodsWarehouse.type === 'goods' && isGoodsType && hide_function.is_show_ecard && !is_mch" src="statics/img/mall/goods.png"></image>
                                    <image style="width: 24px;height: 24px;position: absolute;z-index: 2" v-if="scope.row.goodsWarehouse.type === 'ecard' && isGoodsType && hide_function.is_show_ecard && !is_mch" src="statics/img/mall/ecard-goods.png"></image>
                                    <image style="width: 24px;height: 24px;position: absolute;z-index: 2" v-if="scope.row.goodsWarehouse.type === 'form-goods' && isGoodsType && hide_function.is_show_form_goods" src="statics/img/mall/form-goods.png"></image>
...
<script>
    Vue.component('app-goods-list', {
        ...
        created() {
            ...
            this.placeholder = this.placeHolder ? this.placeHolder :'请输入'+this.text+'ID、货号或名称搜索'
            // 搜索条件从缓存中获取
            let search = localStorage.getItem('GOODS_SEARCH');
            if (search) {
        ...
        methods: {
            ...
            getList() {
                ...
                localStorage.setItem('GOODS_SEARCH', JSON.stringify(this.search));
                request({
                    params: {
                        r: self.goods_url,
                        page: self.page,
