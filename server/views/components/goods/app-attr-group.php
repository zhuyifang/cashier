    ...
    <div class="app-attr-group">
        <div v-for="(group, i) in value" :key="i" style="margin-bottom: 16px">
            <div flex="dir:left cross:center" class="bg">
                <span>规格名:</span>
                <el-input type="text" v-model.trim="group.attr_group_name" size="mini" @change="makeAttrGroup"
            ...
            <div flex="dir:left" style="padding:0 10px;margin-top: 16px">
                ...
                <draggable :options="{draggable:'.attr-list'}" @end="makeAttrGroup"
                    ...
                    <div class="attr-list" v-for="(attr, j) in group.attr_list" :key="j">
                        <el-input style="width:152px" size="mini" type="text" @change="makeAttrGroup"
        ...
        <div v-if="isAddAttrGroups" flex="dir:left cross:center" class="bg">
            ...
            <span style="padding-left: 14px;color:#c9c9c9">注：规格名最多添加{{attrGroupMaxCount}}个</span>
        </div>
    </div>
</template>
<script>
    Vue.component('app-attr-group', {
        ...
        watch: {
            'appGoods.ruleForm.type': {
                handler(newData) {
                    let {type, form_mode_type} = this.appGoods.ruleForm;
                    if(type === 'form-goods' && form_mode_type === 'calendar'){
                        this.attrGroupMaxCount = 2;
                    } else {
                        this.attrGroupMaxCount = 5;
                    }
                },
                immediate: true,
            },
            'appGoods.ruleForm.form_mode_type': {
                handler(newData) {
                    let {type, form_mode_type} = this.appGoods.ruleForm;
                    if(type === 'form-goods' && form_mode_type === 'calendar'){
                        this.attrGroupMaxCount = 2;
