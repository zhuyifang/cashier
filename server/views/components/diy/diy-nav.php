                ...
                <div v-if="data.navType === 'fixed'" flex="dir:top" class="nav-fixed"
                    ...
                    <div v-for="(navs, index1) in newNavs" :key="index1" flex="dir:left">
                        <div v-for="(nav, index2) in navs" :key="index2"
                             :style="[beforeMargin(index2,navs)]"
                             :class="{'box-grow-1': data.modeType === 'image'}">
                            <div flex="dir:top main:center cross:center" class="nav-list"
                                 :style="{width: data.modeType === 'image' ? '100%': '100px'}"
                                 :class="{'nav-list-text' : data.modeType === 'text'}">
                                <image v-if="data.modeType === 'image' && nav.icon" class="nav-image box-grow-1"
                                       :style="{height: data.columns == 2 ? '200px': '130px', width: data.columns == 2 ? '340px': '222px'}"
                                       :src="nav.icon"></image>
                                <image v-if="data.modeType === 'img' && !nav.temp" class="nav-image"
                                       :src="nav.icon ? nav.icon : 'statics/img/mall/default_img.png'"></image>
                                <div v-if="nav.name && data.modeType !== 'image'" class="nav-name" :style="{color: data.color}"
                ...
                <div v-if="data.navType === 'alone'" flex style="overflow:auto">
                    <div flex="dir:left" class="nav-alone" style="overflow:visible">
                        <div flex="dir:top main:center cross:center" v-for="(nav, index) in data.navs" :key="index" :style="[formatB(index)]" class="box-list">
                            <image v-if="data.modeType === 'image' && nav.icon" class="nav-image box-grow-1"
                                   :style="{height: `${data.imgHeight}px`, width: '100%'}"
                                   :src="nav.icon"></image>
            ...
            <el-form label-width="100px" style="padding: 20px 0">
                <el-form-item label="选择模块">
                    <div flex="dir:left">
                        <div flex="dir:top cross:center main:center"
                            ...
                            <div :style="{color: `${data.modeType == 'img' ? '#409EFF':'#666666'}`}">图标导航</div>
                        </div>
                        <div flex="dir:top cross:center main:center"
                        ...
                        <div flex="dir:top cross:center main:center"
                             style="cursor: pointer;border-radius:3px;margin-left: 16px"
                             @click="up('image')"
                             :style="{border: `1px solid ${data.modeType == 'image' ? '#409EFF':'#DDDDDD'}`}">
                            <div style="width: 100px;height: 48px" flex="cross:center main:center">
                                <img src="statics/img/mall/diy/nav_image.png" alt="" style="width: 100%;height: 36px">
                            </div>
                            <div :style="{color: `${data.modeType == 'image' ? '#409EFF':'#666666'}`}">图片导航</div>
                        </div>
                    </div>
                </el-form-item>
                <el-form-item label="滑动设置">
                    <app-radio turn v-model="data.navType" label="fixed">固定</app-radio>
                    <app-radio turn v-model="data.navType" label="alone">单行滑动</app-radio>
                    <app-radio turn v-if="data.modeType != 'image'" v-model="data.navType" label="multi">多行滑动</app-radio>
                </el-form-item>
                <el-form-item label="每行个数" v-if="data.navType === 'fixed' || data.navType === 'multi'">
                    <el-select v-model="data.columns" placeholder="请选择">
                        <el-option v-if="data.modeType === 'image'" label="两个导航" :value="2"></el-option>
                        <el-option label="三个导航" :value="3"></el-option>
                        <el-option v-if="data.modeType !== 'image'" label="四个导航" :value="4"></el-option>
                        <el-option v-if="data.modeType !== 'image'" label="五个导航" :value="5"></el-option>
                        <el-option v-if="data.modeType !== 'image'" label="六个导航" :value="6"></el-option>
                    </el-select>
                </el-form-item>
                <template v-if="data.modeType !== 'image'">
                <el-form-item label="一屏行数" v-if="data.navType === 'multi'">
                    <el-select v-model="data.lineNum" placeholder="请选择">
                        ...
                        <el-option v-if="data.modeType !== 'image'" label="四行" :value="4"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="颜色设置">
                    <app-radio turn v-model="data.bgType" label="color">颜色</app-radio>
                    <app-radio turn v-model="data.bgType" label="pic">背景图</app-radio>
                </el-form-item>
                ...
                <el-form-item label="文字颜色" v-if="data.modeType !== 'image'">
                    <el-color-picker @change="(row) => {row == null ? data.color = '#353535' : ''}" size="small"
                                     v-model="data.color"></el-color-picker>
                    <el-input size="small" style="width: 64px;margin-right: 25px;" v-model="data.color"></el-input>
                </el-form-item>
                <?php if ($mchId == 0): ?>
                    ...
                    <draggable class="goods-list" style="width: 355px;max-height: 600px;overflow-y: auto;padding-right: 25px;" flex="dir:top" v-model="data.navs"
                        ...
                        <div v-for="(nav,index) in data.navs" class="edit-nav-item drag-drop">
                            ...
                            <div flex="dir:left box:first cross:center">
                                ...
                                <div style="flex-grow: 0" v-if="data.modeType === `image`">
                                    <app-image-upload style="margin-right: 5px;" v-model="nav.icon" width="340"
                                                      height="200"></app-image-upload>
                                </div>
                                <div style="flex-grow: 1;max-width: 100%" @click="pickLinkClick(index)">
                                    <el-input class="item-drag" v-model="nav.name" placeholder="请输入名称，最多输入5个字"
                                              size="small"
                                              v-if="data.modeType !== `image`"
                                              style="margin-bottom: 5px" maxlength="5"></el-input>
                                    <app-pick-link @selected="linkSelected" :show-customer="false" :link="nav.link">
                                    </app-pick-link>
...
<!--                         <el-pagination
                ...
                <el-form-item label="图片高度" v-if="data.navType === 'alone' && data.modeType === 'image'">
                    <div flex="dir:left cross:center">
                        <el-slider :show-tooltip="false" v-model="data.imgHeight" style="width: 100%" size="small"
                                   :max="200"
...
<script>
    Vue.component('diy-nav', {
        ...
        computed: {
            ...
            formatB() {
                return (index) => {
                    if(this.data.modeType === 'image'){
                        let w = this.data.imgHeight * 1.7;
                        let extra = {width: `${w}px`,'min-width': `${w}px`,marginLeft: '20px'};
                        if(index === 0){
                            Object.assign(extra, {marginLeft: '24px'})
                        } else if(index === this.data.navs.length - 1){
                            Object.assign(extra, {marginRight: '24px'})
                        }
                        return extra;
                    }
                    let aloneNum = Number(this.data.aloneNum);
        ...
        methods: {
            beforeMargin(index2,navs){
                if(this.data.modeType === 'image'){
                    return {
                        width: '50%',
                        marginLeft: index2 === 0 ? '24px' : '20px',
                        marginRight: index2 === navs.length - 1 ? '24px' : '0'
                    }
                } else {
                    return {
                        marginRight: index2 === navs.length - 1 ? 'auto' : '0',
                        marginLeft: 'auto',
                        width: 'auto'
                    }
                }
            ...
            up(info){
                this.data.modeType = info;
                if(info === 'image'){
                    if(this.data.navType === 'multi') this.data.navType = 'fixed';
                    if(this.data.columns == 4 || this.data.columns == 5  || this.data.columns == 6 ) this.data.columns = 2;
                } else {
                    if(this.data.columns == 2 ) this.data.columns = 3;
                }
            },
            selectBg(e) {
                if (e.length) {
            ...
            linkSelected(list, params) {
                ...
                if (this.currentEditNavIndex !== null) {
                    ...
                    this.data.navs[this.currentEditNavIndex].link = link;
