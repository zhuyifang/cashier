...
Yii::$app->loadViewComponent('goods-calendar');
...
<style>
    ...
    .app-goods .input-item-goods {
        display: inline-block;
        width: 250px;
        margin: 0 0 20px;
    }

    .app-goods .input-item-goods .el-input__inner {
        border-right: 0;
    }

    .app-goods .input-item-goods .el-input__inner:hover {
        border: 1px solid #dcdfe6;
        border-right: 0;
        outline: 0;
    }

    .app-goods .input-item-goods .el-input__inner:focus {
        border: 1px solid #dcdfe6;
        border-right: 0;
        outline: 0;
    }

    .app-goods .input-item-goods .el-input-group__append {
        background-color: #fff;
        border-left: 0;
        width: 10%;
        padding: 0;
    ...
    .app-goods .input-item-goods .el-input-group__append .el-button {
        padding: 0;
    }
    ...
    .app-goods .limit-buy .el-form-item__label {
        text-align: left;
    }
    ...
    .app-goods .cishu .el-radio__input.is-checked + .el-radio__label {
        color: #606266;
    }

    .app-goods .cishu .el-radio {
        margin-right: 0;
    }
    ...
    .app-goods .cishu > div:last-of-type {
        margin-bottom: 0;
    }

    .app-goods .cishu .gudingyunfei .el-radio__label {
        display: none;
    }

    .app-goods .cishu .gudingyunfei .el-radio {
        margin-right: 5px;
    }

    .app-goods .required-icon .el-form-item__label:before {
        content: '*';
        color: #F56C6C;
        margin-right: 4px;
    }

    .app-goods .el-card__header {
        background-color: #F9F9F9;
    }

    .app-goods-tabs--head.el-tabs.el-tabs--top {
        background: #f3f3f3;
    }

    .app-goods-tabs--head .el-tabs__header.is-top {
        background: white;
        padding: 0 20px;
    }

    .app-goods-tabs--head > .el-tabs__content {
        background: white;
        padding: 20px 20px 0;
    }

    .form-goods-icon {
        background: url('statics/img/mall/form-goods/load.gif') no-repeat;
        font-size: 107px;
        background-size: cover;
    }

    .form-goods-icon:before {
        content: "替";
        color: rgba(0, 0, 0, 0);
        font-size: 141px;
    }
...
<template id="app-goods">
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" class="app-goods"
        ...
        <div style="height: 100vh; position: fixed; width: calc(100vw - 305px); z-index: 3; top: 55px;"
             v-if="btnLoading" v-loading="btnLoading">
        </div>
        <div class='form-body'>
            <el-form :model="cForm" :rules="cRule" ref="ruleForm" label-width="180px" size="small"
                     class="demo-ruleForm">
                <el-tabs class="app-goods-tabs--head" v-model="activeName" @tab-click="handleClick"
                         :before-leave="leaveTab">
                    <el-tab-pane label="商品设置" name="first" v-if="is_basic == 1">
                        <el-card v-if="(is_ecard == 1 && is_e_card == 1) || isShowCalendar" shadow="never" class="mt-24">
                            <div slot="header">
                                <span>选择商品类型</span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <div flex="">
                                        <image @click="setEcard('goods')" class="goods-types"
                                               src="statics/img/mall/goods/good-types.png"></image>
                                        <image @click="setEcard('ecard')" class="goods-types" v-if="is_mch == 0"
                                               src="statics/img/mall/goods/ecard-types.png"></image>
                                        <image @click="setEcard('form-goods')" class="goods-types" v-if="isShowCalendar"
                                               src="statics/img/mall/goods/form-goods.png"></image>
                                        <image class="goods-types active"
                                               :style="{left: ruleForm.type != 'goods' ? ruleForm.type == 'ecard' || is_mch != 0  ? '223px' :'446px' : '0px'}"
                                               src="statics/img/mall/goods/active-types.png"></image>
                                    </div>
                                </el-col>
                            </el-row>
                        </el-card>
                        <el-card v-if="ruleForm.type == 'ecard' && is_e_card == 1 && is_mch == 0 && is_ecard == 1"
                                 shadow="never" class="mt-24">
                            <div slot="header">
                                <span>选择卡密</span>
                            </div>
                            <el-row>
                                    ...
                                    <el-form-item label="卡密名称" required>
                                        <div flex="dir:left">
                                            <div flex="dir:top" v-if="ruleForm.plugin_data.ecard.name">
                                                ...
                                                <span style="color: #ff4544;font-size: 14px;"
                                                      v-if="ruleForm.plugin_data.ecard.stock < 50">库存少于50</span>
                                            </div>
                                            <div>
                                                <el-button :disabled="is_edit == 1" type="primary"
                                                           @click="$refs.ecard.openDialog()">选择卡密
                                                </el-button>
                                            </div>
                                            <div style="margin-left: 10px;">
                                                <el-button type="text"
                                                           @click="$navigate({r:'plugin/ecard/mall/index/edit'}, true)">
                                                    添加卡密
                                                </el-button>
                                            </div>
                            ...
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <template v-if="is_info == 1 || sign === 'exchange'">
                                        ...
                                        <!-- 关键词 -->
                                        <el-form-item label="关键词" v-if="is_name === 1" prop="keyword"
                                                      class="title-input"
                                        >
                                        ...
                                        <el-form-item label="商品信息获取" width="120" v-if="is_product_info === 1">
                                            ...
                                            <div>
                                                <el-row type="flex">
                                                    ...
                                                    <app-select-goods :multiple="false" submit_text="选择" :sign="sign"
                                                                      v-if="is_edit == 0"
                                                                      @selected="selectGoodsWarehouse">
                                                        <el-button>选择商品</el-button>
                                                    </app-select-goods>
                                                </el-row>
                                                <el-button v-if="is_edit == 0" type="text"
                                ...
                                <el-col :xl="12" :lg="16">
                                    ...
                                    <el-form-item label="售价" prop="price" v-if="no_price === 1">
                                        <el-input type="number"
                                                  :disabled="is_info == 0 && sign != 'pintuan' && sign !== 'flash_sale' && sign !== 'wholesale' && sign !== 'exchange' && sign != 'weekly_buy'"
                                    ...
                                    <!-- 原价 -->
                                    <el-form-item v-if="sign !== 'exchange'" prop="original_price"
                                                  :required="sign ? false : true">
                                        <template slot="label">
                                            <span>原价</span>
                                            <el-tooltip effect="dark" content="以划线形式显示" placement="top">
                                                <i class="el-icon-info"></i>
                                    ...
                                    <el-form-item
                                            v-if="cForm.extra && sign && sign !== 'integral_mall' && sign !== 'miaosha' && sign !== 'booking' && sign !== 'step' && sign !== 'community'"
                                            v-for="(item, key, index) in cForm.extra"
                                            :key="item.id">
                                    ...
                                    <template v-if="is_attr == 1">
                                        ...
                                        <el-form-item v-if="ruleForm.type == 'goods'  || ruleForm.type === 'form-goods'"
                                                      label="默认规格名" prop="attr_default_name">
                                        ...
                                        <el-form-item v-if="ruleForm.type == 'goods' || ruleForm.type === 'form-goods'">
                                            ...
                                            <div v-if="isShowCalendar && ruleForm.type === 'form-goods'"
                                                 style="margin-bottom: 15px">
                                                <el-radio v-model="ruleForm.form_mode_type" label="basic" turn>传统模式
                                                    <el-radio style="margin-left: 56px"
                                                              v-model="ruleForm.form_mode_type" label="calendar" turn>
                                                        日历化模式
                                                    </el-radio>
                                            </div>
                                            <div style="width:150%">
                                            ...
                                            <div v-if="ruleForm.use_attr" style="width:130%;margin-top: 24px;">
                                                <app-attr v-model="ruleForm.attr" :attr-groups="attrGroups"
                                                          ref="appattr"
                                                          @upattr="handleAttrData"
                                                          :required-extra="extra_attr_require"
                                                          :goods_type="ruleForm.type"
                                                          :showtype="ruleForm.form_mode_type === 'calendar' ? 'text' : 'after'"
                                                          :extra="cForm.extra ? cForm.extra : {}"></app-attr>
                                            </div>
                                            <template
                                                    v-if="isShowCalendar && ruleForm.type === 'form-goods' && ruleForm.form_mode_type == 'calendar'">
                                                <div style="border: 1px solid #DCDFE6;margin-top: 15px;border-radius: 3px;padding-top:20px;background: #FFFFFF;width: 150%;">
                                                    <el-form-item label="时间段:" required>
                                                        <div flex="dir:left cross:center">
                                                            <el-date-picker
                                                                    v-model="ruleForm.calendar_start"
                                                                    type="date"
                                                                    format="yyyy-MM-dd"
                                                            ...
                                                            <div style="margin:0 10px;color: #606266;font-size: 14px;">至
                                                            </div>
                                                            <el-date-picker
                                                                    v-model="ruleForm.calendar_end"
                                                                    size="small"
                                                                    format="yyyy-MM-dd"
                                                                    value-format="yyyy-MM-dd"
                                                                    style="width: 230px"
                                                        ...
                                                        <div style="font-size: 12px;color:#FF4544">注：请先选择时间段再去设置具体日期的库存和售价</div>
                                                    </el-form-item>
                                                    <el-form-item label="用户可选择开始时间:">
                                                        <div flex="dir:left cross:center">
                                                            <el-radio v-model="ruleForm.is_today" label="none" turn>无限制
                                                            </el-radio>
                                                            <el-radio v-model="ruleForm.is_today" label="today" turn>当天
                                                            </el-radio>
                                                            <el-radio v-model="ruleForm.is_today" label="after"
                                                                      turn></el-radio>
                                                            <div style="margin-left: -62px">
                                                                <el-input v-model="ruleForm.after_day" size="small"
                                                    ...
                                                    <el-form-item label="日期选项">
                                                        <el-radio v-model="ruleForm.is_alone" :label="1" turn>单日期
                                                        </el-radio>
                                                        <el-radio v-model="ruleForm.is_alone" :label="0" turn>多日期
                                                        </el-radio>
                                                    </el-form-item>
                                                    <el-form-item label="天数计算" v-if="ruleForm.is_alone == 0">
                                                        <el-radio @click.navtive="changeMin"
                                                                  v-model="ruleForm.has_kuatian"
                                                                  :label="0" turn>当天
                                                        </el-radio>
                                                        <el-tooltip
                                                                style="position: relative;left: -25px;top:1px;cursor: pointer;color:#606266"
                                                                effect="dark" content="示例：如用户选择1号至7号  当天计算为7晚/天"
                                                                placement="top">
                                                            <i class="el-icon-info"></i>
                                                        </el-tooltip>
                                                        <el-radio @click.navtive="changeMax"
                                                                  v-model="ruleForm.has_kuatian"
                                                                  :label="1" turn>跨天
                                                        </el-radio>
                                                        <el-tooltip
                                                    ...
                                                    <el-form-item label="限定天数" v-if="ruleForm.is_alone == 0">
                                                        <el-switch v-model="ruleForm.is_day" :active-value="1"
                                                                   style="margin-bottom: 5px"
                                                                   :inactive-value="0"></el-switch>
                                                        <div v-if="ruleForm.is_day == 1">
                                                            <el-input-number v-model="ruleForm.day_max"
                                                                             size="small"
                                                                             :precision="0"
                                                                             :step="1"
                                                                             :min="ruleForm.has_kuatian == 1 ? 1 : 2"
                                                                             :max="day_max_max"
                                                            ></el-input-number>
                                                            <span style="margin-left: 10px">{{ruleForm.place_unit}}</span>
                                                        </div>
                                                    </el-form-item>
                                                    <el-form-item label="提示单位" v-if="ruleForm.is_alone == 0"
                                                                  style="margin-right: 10px" required>
                                                        <el-input v-model="ruleForm.place_unit"
                                                                  size="small"
                                                                  maxlength="1"
                                                                  style="width: 293px"
                                                                  show-word-limit
                                                ...
                                                <goods-calendar ref="goods_calendar" @flushrows="getRange"
                                                                showtype="basic" style="width: 150%"></goods-calendar>
                                            </template>
                                        </el-form-item>
                                        <el-form-item v-if="is_show == 1 && !is_mch && ruleForm.type == 'goods'"
                                                      prop="is_negotiable">
                                            <template slot='label'>
                                                <span>商品面议</span>
                                        ...
                                        <el-form-item label="商品重量"
                                                      v-if="ruleForm.type == 'goods'  || ruleForm.type === 'form-goods'">
                                    ...
                                    <el-form-item label="选择规格" v-if="is_attr == 0 && sign !== 'exchange'" required>
                                        <el-row type="flex">
                                            <el-select v-model="select_attr_groups" @change="selectAttrGroups"
                                                       placeholder="请选择" v-if="is_edit == 0">
                                                <el-option
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_purchase_settings === 1">
                            ...
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    ...
                                    <el-form-item label="发货方式" prop="is_setting_send_type"
                                                  v-if="is_setting_send_type && (ruleForm.type == 'goods' || ruleForm.type === 'form-goods')">
                                        ...
                                        <div>
                                            ...
                                            <el-checkbox-group v-model="ruleForm.send_type" :min="1"
                                                               :max="send_type_desc.length"
                                                               v-if="ruleForm.is_setting_send_type == 1">
                                                <el-checkbox v-for="item in send_type_desc" :label="item.key">
                                                    {{item.modify ? item.modify : item.origin}}{{item.origin !==
                                                    item.modify && item.modify ? `(${item.origin})` : ''}}
                                                </el-checkbox>
                                            </el-checkbox-group>
                                        </div>
                                    </el-form-item>
                                        ...
                                        <div>
                                            ...
                                            <div v-if="ruleForm.is_time == 1">
                                                ...
                                                <div style="color: #CCCCCC">开启开售提醒，商品开售前5分钟会发消息提醒买家。
                                                    <el-button v-if="is_mch == 0" type="text"
                                                               @click="$navigate({r:'mall/index/goods'}, true)">去设置
                                                    </el-button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <el-form-item prop="virtual_sales" v-if="is_virtual_sales === 1">
                                        ...
                                        <el-input type="text" maxLength="7"
                                                  oninput="this.value = this.value.replace(/[^0-9]/, '')" min="0"
                                                  v-model="ruleForm.virtual_sales">
                                            <template slot="append">{{ruleForm.unit}}</template>
                                        </el-input>
                                    </el-form-item>
                                    <el-form-item label="浏览和购买权限" prop="is_setting_show_and_buy_auth"
                                                  v-if="is_show_and_buy_auth">
                                        <div class="limit-buy">
                                            <el-switch
                                    ...
                                    <el-form-item label="限购" prop="limit_buy_status" v-if="is_limit_buy">
                                        <div class="limit-buy">
                                            ...
                                            <el-form-item :label="sign == 'miaosha' ? '限购' : '终身限购'"
                                                          prop="confine_count" label-width="90px"
                                            ...
                                            <el-form-item label="按周期限购" prop="limit_buy_value" label-width="90px"
                                                ...
                                                <div flex="dir:left">
                                                            ...
                                                            <el-input
                                                                ...
                                                                <template slot="prepend">
                                                                    <el-select size="small" style="width: 120px"
                                                                               v-model="ruleForm.limit_buy_type"
                                                                               class="select">
                                                                        <el-option key="day" label="每天"
                                                                                   value="day"></el-option>
                                                                        <el-option key="week" label="每周"
                                                                                   value="week"></el-option>
                                                                        <el-option key="month" label="每月"
                                                    ...
                                                    <el-checkbox style="margin-left: 5px;"
                                                                 @change="limitBuyValueChecked"
                                                                 v-model="ruleForm.limit_buy_value <= -1">无限制
                                                    </el-checkbox>
                                                </div>
                                            </el-form-item>
                                            <el-form-item label="按订单限购" prop="limit_buy_order" label-width="90px"
                                                          v-if="ruleForm.limit_buy_status == 1 && sign != 'miaosha'">
                                                <div flex="dir:left">
                                                    <template v-if="ruleForm.limit_buy_order > -1">
                                                        <div style="width: 100%;margin:0 10px 0 0">
                                                            <el-input
                                                                    oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                                    :disabled="ruleForm.limit_buy_order <= -1"
                                                                    :min="2"
                                                                    :max="9999"
                                                                    placeholder="请输入1～9999之间的整数"
                                                                    v-model="ruleForm.limit_buy_order">
                                                                <template slot="prepend">
                                                                    <div>每单限购</div>
                                                                </template>
                                                                <template slot="append">件</template>
                                                    ...
                                                    <el-checkbox style="margin-left: 5px;"
                                                                 @change="limitBuyOrderValueChecked"
                                                                 v-model="ruleForm.limit_buy_order <= -1">无限制
                                                    </el-checkbox>
                                                </div>
                                            </el-form-item>
                                        </div>
                                    ...
                                    <el-form-item v-if="sign !== 'exchange'" label="商品服务标识">
                                        <el-row>
                                            <el-col :span="4">
                                                <app-attachment v-model="ruleForm.guarantee_pic" :multiple="false"
                                                                :max="1">
                                            ...
                                            <el-col :span="4">
                                                <el-button size="mini" type="primary"
                                                           @click="ruleForm.guarantee_pic = guarantee.goods_pic">恢复默认
                                                </el-button>
                                            </el-col>
                                        </el-row>
                                        ...
                                        <template v-if="!defaultServiceChecked">
                                            ...
                                            <el-dialog title="选择商品服务" :visible.sync="service.dialog" width="30%">
                                                <el-card shadow="never" style="max-height: 488px;overflow: auto"
                                                    ...
                                                    <el-checkbox-group v-model="service.list" flex="dir:left"
                                    ...
                                    <el-form-item required
                                                  v-if="(ruleForm.type == 'goods' || ruleForm.type === 'form-goods') && is_shipping">
                                        <template slot='label'>
                                            <span>运费设置</span>
                                        ...
                                        <div v-if="sign == 'weekly_buy'" class="cishu">
                                            <div class="gudingyunfei">
                                                <el-radio v-model="form.freight_type" :label="0"></el-radio>
                                                ...
                                                <span style="margin-left: 10px;color: #C0C4CC;font-size: 12px">注：总运费=期数*每期运费；同城配送总运费均为同城配送设置的运费*期数</span>
                                            </div>
                                            <div>
                                                <el-radio v-model="form.freight_type" :label="1">固定运费</el-radio>
                                                <div flex="dir:left cross:center"
                                                    ...
                                                    <div>
                                                        <el-input
                                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                                :min="0" type="number"
                                                                style="width: 214px;margin: 0 10px;"
                                                    ...
                                                    <div>
                                                        <el-input
                                                                ...
                                                                :min="0" type="number"
                                                                style="width: 214px;margin: 0 10px;"
                                                                v-model="form.city_freight_price">
                                                            <template slot="append">元</template>
                                                        </el-input>
                                                    </div>
                                                </div>
                                        ...
                                        <template v-else>
                                            ...
                                            <el-button v-else type="button" size="mini" @click="freightOpen">选择运费
                                            </el-button>
                                        </template>
                                        <el-dialog title="选择运费" :visible.sync="freight.dialog" width="30%">
                                            <div class="input-item-goods">
                                                <el-input size="small" placeholder="请输入运费名称" v-model="search.ssYF"
                                                          clearable>
                                            ...
                                            <el-card shadow="never" flex="dir:left" style="flex-wrap: wrap"
                                                ...
                                                <el-radio-group v-if="YFlist && YFlist.length"
                                                                v-model="freight.checked">
                                                    <el-radio style="padding: 10px;" v-for="item in YFlist"
                                                              :label="item" :key="item.id">{{item.name}}
                                                    </el-radio>
                                                </el-radio-group>
                                                <div v-else style="width: 520px;color:#999999"
                                                     flex="cross:center main:center">暂无搜索结果
                                                </div>
                                            </el-card>
                                            <div slot="footer" class="dialog-footer">
                                    ...
                                    <!-- 包邮规则 -->
                                    <el-form-item  v-if="(ruleForm.type == 'goods' || ruleForm.type === 'form-goods') && is_shipping_rules　&& sign != 'weekly_buy'">
                                        <template slot='label'>
                                            <span>是否包邮</span>
                                        </template>
                                        <el-switch
                                                v-model="ruleForm.is_shipping"
                                                :active-value="1"
                                                :inactive-value="0">
                                        </el-switch>
                                    </el-form-item>
                                    <el-form-item required v-if="(ruleForm.type == 'goods' || ruleForm.type === 'form-goods') && is_shipping_rules　&& ruleForm.is_shipping == 1">
                                        <template slot='label'>
                                            <span>包邮规则</span>
                                        </template>
                                        <div v-if="sign == 'weekly_buy'" class="cishu">
                                            <div>
                                                <el-radio v-model="form.shipping_type" :label="0">不包邮</el-radio>
                                            </div>
                                            <div>
                                                <el-radio v-model="form.shipping_type" :label="1">快递配送满</el-radio>
                                                <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                          :min="0" type="number" style="width: 220px;margin: 0 10px;"
                                                ...
                                                <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                          :min="0" type="number" style="width: 220px;margin: 0 10px;"
                                                          v-model="form.city_shipping_number">
                                                    <template slot="append">期</template>
                                                </el-input>
                                                包邮
                                            </div>
                                            <div>
                                                <el-radio v-model="form.shipping_type" :label="2">快递配送满</el-radio>
                                                <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                          :min="0" type="number" style="width: 220px;margin: 0 10px;"
                                                ...
                                                <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                          :min="0" type="number" style="width: 220px;margin: 0 10px;"
                                                          v-model="form.city_shipping_price">
                                                    <template slot="append">元</template>
                                                </el-input>
                                                包邮
                                            </div>
                                    ...
                                    <!-- 自定义表单 -->
                                    <el-form-item prop="is_form" v-if="is_form == 1">
                                        <template slot='label'>
                                            <span>自定义表单</span>
                                    ...
                                    <el-form-item label="区域购买" prop="is_area_limit"
                                                  v-if="is_area_limit == 1 &&(ruleForm.type == 'goods' || ruleForm.type === 'form-goods') && sign !='community'">
                                        <el-switch
                                    ...
                                    <el-form-item v-if="is_area_limit == 1 && ruleForm.is_area_limit" label="允许购买区域"
                                                  prop="area_limit">
                    ...
                    <el-tab-pane label="分销价设置" name="there" v-if="is_show_share">
                        <el-form-item label="是否参与分销" prop="is_share" v-if="is_mch == 0">
                            <el-switch :active-value="1" :inactive-value="0" v-model="ruleForm.is_share">
                            </el-switch>
                        </el-form-item>
                        <el-form-item :label="is_mch ? '是否开启分销佣金' : '是否开启单独分销'" prop="individual_share"
                                      v-if="ruleForm.is_share == 1 || is_mch > 0">
                            <el-switch :active-value="1" :inactive-value="0" v-model="ruleForm.individual_share">
                            </el-switch>
                        </el-form-item>
                        <template v-if="ruleForm.individual_share == 1 && (ruleForm.is_share == 1 || is_mch > 0)">
                            <el-form-item label="分销类型" prop="attr_setting_type"
                                          v-if="cForm.use_attr == 1 && (ruleForm.type == 'goods' || ruleForm.type == 'form-goods')">
                            ...
                            <template
                                    v-if="isShowCalendar && ruleForm.type === 'form-goods' && ruleForm.form_mode_type == 'calendar'">
                                <el-form-item label="规格化日历设置">
                                    <goods-calendar showtype="sharePrice"></goods-calendar>
                                </el-form-item>
                            </template>
                            <template v-else-if="sign !== 'pintuan' && sign !== 'weekly_buy'">
                            ...
                            <template v-if="sign === 'pintuan' || sign === 'weekly_buy'">
                                ...
                                <template v-if="ruleForm.group_list.length > 0">
                                    <div class="pintuan-share">
                                        <el-card v-for="(item, index) in ruleForm.group_list" :key="index">
                                            <app-goods-share v-model="ruleForm.group_list[index]"
                                                             :attr-groups="attrGroups"
                                                             :attr_setting_type="ruleForm.attr_setting_type"
                                                             :share_type="ruleForm.share_type"
                                                             :use_attr="ruleForm.use_attr"
                                                             :sign="sign"
                                                             :pintuan_sign="sign === 'weekly_buy' ? item.number + '期' : item.people_num + '人团'"
                                            ></app-goods-share>
                                        </el-card>
                                    </div>
                                </template>
                                <template v-else>
                                    <el-form-item>
                                        <el-tag type="danger">请先添加{{sign === 'weekly_buy' ? '周期' : '阶梯团'}}</el-tag>
                                    </el-form-item>
                                </template>
                            </template>
                        </template>
                    ...
                    <el-tab-pane label="会员价设置" name="third" v-if="is_member === 1">
                        ...
                        <template v-if="ruleForm.is_level == 1">
                            ...
                            <template
                                    v-if="ruleForm.is_level_alone == 1 && isShowCalendar && ruleForm.type === 'form-goods' && ruleForm.form_mode_type == 'calendar'">
                                <el-form-item label="规格化日历设置">
                                    <goods-calendar showtype="memberPrice"></goods-calendar>
                                </el-form-item>
                            </template>
                            <template v-else-if="ruleForm.is_level_alone == 1">
                                ...
                                <template v-if="sign !== 'pintuan' && sign !== 'weekly_buy'">
                                    ...
                                    <template v-if="ruleForm.use_attr == 0 && memberLevel.length > 0">
                                        <el-form-item label="默认规格会员价设置">
                                            <el-col :xl="12" :lg="16">
                                                <el-input v-for="item in defaultMemberPrice" :key="item.id"
                                                          type="number"
                                                ...
                                                <template v-if="ruleForm.use_attr == 1 && memberLevel.length > 0">
                                                    <div flex="dir:top">
                                                        <div style="margin-left: 180px; padding: 16px; border-left: 1px solid #EBEEF5;border-right: 1px solid #EBEEF5;border-top: 1px solid #EBEEF5;">
                                                            <el-tag style="float: left;margin-right: 50px"
                                                                    type="danger">
                                                                {{sign === 'weekly_buy' ? item.number + '期' :
                                                                item.people_num + '人团'}}
                                                            </el-tag>
                                                        </div>
                                                        <!--多规格会员价设置-->
                                                ...
                                                <template v-if="ruleForm.use_attr == 0 && memberLevel.length > 0">
                                                    <div flex="dir:top" class="member-level">
                                                        <el-form-item class="member-item">
                                                            <el-col :xl="12" :lg="16"
                                                                    style="padding: 16px;border-left: 1px solid #EBEEF5;border-right: 1px solid #EBEEF5;border-top: 1px solid #EBEEF5;">
                                                                <el-tag style="float: left;margin-right: 50px"
                                                                        type="danger">
                                                                    {{sign === 'weekly_buy' ? item.number + '期' :
                                                                    item.people_num + '人团'}}
                    ...
                    <el-tab-pane v-if="ruleForm.type === 'form-goods'" label="定制设置" v-if="isShowCalendar" name="four">
                        <div>
                            <el-form-item label="定制化设置" prop="customization_status" style="margin-left: -53px">
                                <el-switch :active-value="1" :inactive-value="0"
                                           v-model="ruleForm.customization_status">
                                </el-switch>
                            </el-form-item>
                        </div>
                        <template v-if="ruleForm.customization_status == 1">
                            <div style="background:#f3f3f3;margin:0 -20px;height: 10px;width: calc(100% + 40px)"></div>
                            <form-goods-edit v-model="ruleForm.customization_data" ref="formGoods">
                                <template slot="head">
                                    <div></div>
                                </template>
                                <template slot="end">
    ...
    Vue.component('app-goods', {
        ...
        data() {
            ...
            let rules = {
                ...
                is_setting_show_and_buy_auth: [
                    {required: true, type: 'integer', message: '请选择是否开启', trigger: 'blur'}
                ],
                is_area_limit: [
                    {required: true, type: 'integer', message: '请选择是否开启', trigger: 'blur'}
                ],
                ...
                limit_buy_order: [
                    {
                        type: 'integer', validator: (rule, value, callback) => {
                            if (value > -1 && value < 1) {
                                callback('按订单限购数量最少为1');
                            }
                            if (value > max) {
                                callback('按订单限购数量最大为' + max);
                            }
                            callback();
                        }, trigger: 'change'
                    }
            ...
            return {
                pickerStartAuditYear: {
                    disabledDate: time => {
                        if (this.ruleForm.calendar_end) {
                            let t = new Date(this.ruleForm.calendar_end);
                            let n = new Date(time);
                            return n.getTime() > t.getTime() || n.getTime() + 86400000 * 90 < t.getTime()
                        } else {
                            return false;
                        }
                    }
                ...
                pickerEndAuditYear: {
                    disabledDate: time => {
                        if (this.ruleForm.calendar_start) {
                            let t = new Date(this.ruleForm.calendar_start);
                            let n = new Date(time);
                            return n.getTime() < t.getTime() || n.getTime() - 86400000 * 90 > t.getTime()
                        } else {
                            return false;
        ...
        computed: {
            isShowCalendar() {
                ...
                $is_form_goods = array_search('form_goods', $permission) !== false;
                ?>

                let is_form_goods = "<?= $is_form_goods ?>";
                return is_form_goods && ['mall', 'mch'].indexOf(this.sign) !== -1;
            },
            YFlist() {
                if (this.search.ssYF) {
            ...
            cForm() {
                ...
                if(this.sign != '' && this.sign != 'mch' && this.sign != 'goods'){
        ...
        methods: {
            ...
            store(formName) {
                let self = this;
                let isZero = false;
                if ((self.cForm.type == 'goods' || self.cForm.type === 'form-goods') && self.cForm.plugin_data.ecard) {
                    self.cForm.plugin_data.ecard.ecard_id = 0;
                }
                try {
                    //////更新位置
                    if (this.$refs.appattr) {
                        const attrData =  self.$refs.appattr.attrData;
                        for (let _ in this.ruleForm.attr) {
                            let oo = this.ruleForm.attr[_], obj = {};
                            let otherAttrData = this.otherAttrData;
                            if(this.ruleForm.type == 'goods' || (this.ruleForm.type == 'form-goods' && this.ruleForm.form_mode_type === 'basic')){
                                otherAttrData = ['shareLevelList', 'member_price'].concat(this.otherAttrData);
                            }
                            otherAttrData.forEach(_ => {
                                Object.assign(obj, {
                                    [_]: oo[_]
                                })
                            })
                            this.ruleForm.attr[_] = Object.assign(this.ruleForm.attr[_], {
                                bar_code: '0',
                                cost_price: '0',
                                member_price: {},
                                no: '0',
                                ...
                                stock: '0',
                                weight: '0',
                            }, attrData[JSON.stringify(this.ruleForm.attr[_].attr_list)],obj)
                        }
                    }
                    ///////更新位置
                    ...
                    if (self.sign == 'weekly_buy') {
                        self.cForm.status = self.ruleForm.status;
                        self.cForm.activity_status = self.ruleForm.status;
                        self.cForm.virtual_sales = self.ruleForm.virtual_sales;
                    }
                ...
                self.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        if (self.is_svip) {
                            ...
                            delete self.ruleForm['is_vip_card_goods']
                        }
                        ...
                        if (self.ruleForm.goods_num == '') {
                            self.cForm.goods_num = 0;
                        }
                        let paramForm = {};
                        if (this.$refs.formGoods) {
                            Object.assign(paramForm, {
                                customization_data: this.$refs.formGoods.updateValue(),
                            })
                        }

                        if (this.$refs.goods_calendar) {
                            this.$refs.goods_calendar.attrAddDate();
                        }
                        let newData = JSON.parse(JSON.stringify(self.cForm));
                        ...
                        request({
                            ...
                            method: 'post',
                            data: {
                                form: JSON.stringify(Object.assign(newData, paramForm)),
            ...
            makeAttrGroup(e) {
                ...
                let group_arr = [];
                let group_s = false, attr_s = false;
                self.attrGroups.forEach(function (attrGroupItem, attrGroupIndex) {
                    if (group_arr.indexOf(attrGroupItem.attr_group_name) !== -1) {
                        group_s = true;
                    }
                    group_arr.push(attrGroupItem.attr_group_name);
                    let attr_arr = []
                    attrGroupItem.attr_list.forEach(function (attrListItem, attrListIndex) {
                        if (attr_arr.indexOf(attrListItem.attr_name) !== -1) {
                            attr_s = true;
                        }
                        attr_arr.push(attrListItem.attr_name);
                        let object = {
                ...
                if (group_s) {
                    this.$message.error('规格组名称不能重复');
                } else if (attr_s) {
                    this.$message.error('同一规格组下,规格名称不能重复');
                }
                ...
                // 3.组合结果赋值
                if (this.$refs.goods_calendar) {
                    this.$refs.goods_calendar.makeAttrGroup(res);
                }
                for (let i in res) {
            ...
            setEcard(data) {
                if (this.is_edit == 0) {
                    ...
                    } else if (data === 'form-goods') {
                        if (this.use_attr == 1) {
                            this.ruleForm.use_attr = 1;
                        }
                        if(this.ruleForm.use_attr == 1 && this.attrGroups.length >2){
                            this.$message.error('定制商品只有2种规格组');
                            return;
                        }
                    }
            ...
            limitBuyOrderValueChecked(type) {
                this.ruleForm.limit_buy_order = type ? -1 : 1;
            },
            async getSendType() {
