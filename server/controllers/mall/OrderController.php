...
use app\forms\mall\order\OrderNumFrom;
...
class OrderController extends MallController
{
    ...
    //同城订单取消
    public function actionCityCancel()
    {
        if (\Yii::$app->request->isPost) {
            $form = new OrderCancelForm();
            $form->attributes = \Yii::$app->request->post();
            return $this->asJson($form->cityCancel());
        }
    }
    ...
    public function actionCustomization()
    {
        $form = new OrderForm();
        $form->attributes = \Yii::$app->request->get();
        return $this->asJson($form->orderCustomization());
    }
}
