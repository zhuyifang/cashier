    ...
    public function register()
    {
        \Yii::$app->on(Order::EVENT_CONFIRMED, function ($event) {
            ...
            if (is_numeric($orderAutoSaleTime) && $orderAutoSaleTime >= 0) {
                // 订单过售后
                $saleTime = $orderAutoSaleTime * 86400;
                $saleTime = $saleTime > 0 ? $saleTime : 5;
                $id = \Yii::$app->queue->delay($saleTime)->push(new OrderSalesJob([
                    'orderId' => $event->order->id
                ]));
                CoreQueueData::add($id, $event->order->token);
                $autoSalesTime = strtotime($event->order->confirm_time) + $saleTime;
