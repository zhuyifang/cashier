        ...
        try {
            ...
            foreach ($this->orderDetailList as $orderDetail) {
                ...
                if ($orderDetail->refund && in_array($orderDetail->refund->type, [1, 3]) && $orderDetail->refund->is_refund == 1) {
                    if ($orderDetail->total_price > 0 && $orderDetail->total_price >= $orderDetail->refund->refundGoodsPrice) {
                        $number = ($orderDetail->total_price - $orderDetail->refund->refundGoodsPrice) / $orderDetail->total_price;
    ...
    protected function memberLevel($orderMoneyCount)
    {
        try {
            ...
            if ($nextLevel) {
                ...
                $this->sendTemplate($this->user, '会员等级变更:你的会员等级身份已变更为' . $nextLevel->name);
                $this->sendSmsToUser($this->user, '会员等级', $nextLevel->name);
            }
            return true;
