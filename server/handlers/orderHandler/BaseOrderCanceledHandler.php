...
abstract class BaseOrderCanceledHandler extends BaseOrderHandler
{
    ...
    protected function goodsAddStock($order)
    {
        ...
        $goodsInfo = json_decode($orderDetail[0]['goods_info'], true);

        // 定制库存处理
        if (isset($goodsInfo['goods_attr']['dateList']) && !empty($goodsInfo['goods_attr']['dateList'])) {
            $dateList = $goodsInfo['goods_attr']['dateList'];
            $goodsId = $orderDetail[0]['goods_id'];
            CommonCustomization::getCommon()->updateStock($orderDetail[0]['num'], 'add', $dateList, $goodsId);
        } else {
            $goodsAttrIdList = [];
            $goodsNum = [];
